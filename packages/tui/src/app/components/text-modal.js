...
import { matchesKey, MouseWheelDeltaTracker, visibleWidth, wrapTextWithAnsi } from "../../tui/index.js"
...
export class TextModal {
	...
	maxScroll() {
		return Math.max(0, (this.cachedLines?.length ?? 0) - this.lastBodyRows)
	}

	/** @param {number} value */
	setScroll(value) {
		const next = this.cachedLines
			? Math.max(0, Math.min(value, this.maxScroll()))
			: Math.max(0, value)
	...
	scrollBy(delta) {
		...
		return this.setScroll(this.scroll + Math.trunc(delta))
	}

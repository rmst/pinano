...
class ModelList {
	...
	activateIndex(index) {
		...
		if (!row) return
		this.selectedIndex = Math.max(0, Math.min(index, this.filteredRows.length - 1))
		this.onSelect?.(row)
	}
	...
	moveSelection(delta, options = {}) {
		const next = nextSelectionIndex(this.selectedIndex, this.filteredRows.length, delta, { wrap: options.wrap !== false })
	...
	renderRow(row, selected, width) {
		...
		const cursor = selected ? theme.fg("chromeAccent", "› ") : "  "
		const provider = theme.fg("muted", `[${providerLabel(entry)}]`)
		const check = row.current ? theme.fg("success", " ✓") : ""
		const title = selected ? theme.fg("chromeAccent", entry.id) : entry.id
		const summaryParts = [
			entry.displayName,
	 ...
	 */
	renderFrame(width) {
		if (this.filteredRows.length === 0) return { lines: [theme.fg("muted", "  No matching models")], spans: [] }
		...
		for (let i = startIndex; i < endIndex; i++) {
			...
			const span = clickableRowSpan({
				...
				width,
				component: this,
				id: `model-selector.item.${row.value}`,
				label: row.entry.displayName || row.entry.id,
				metadata: { row, index: i },
 ...
 * @property {(create: (done: () => void) => { component: Component, focus: Component }, options?: { fullscreen?: boolean }) => void} showSelector
 */

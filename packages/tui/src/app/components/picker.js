...
import { SelectList, offsetSpans, truncateToWidth, visibleWidth } from "../../tui/index.js"
...
class PickerModal {
	...
	renderFrame(width) {
		...
		const listFrame = this.list.renderFrame(innerWidth)
		const listLineOffset = lines.length
		const spans = offsetSpans(listFrame.spans, listLineOffset, 2)
		for (const line of listFrame.lines) lines.push(fit(`  ${line}`, modalWidth))

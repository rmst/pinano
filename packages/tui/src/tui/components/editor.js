...
import { offsetSpans } from "../render-frame.js";
import { getSegmenter, isPunctuationChar, isWhitespaceChar, truncateToWidth, visibleWidth } from "../utils.js";
import { SelectList } from "./select-list.js";

/** @typedef {import("../autocomplete.js").AutocompleteProvider} AutocompleteProvider */
/** @typedef {import("../autocomplete.js").AutocompleteSuggestions} AutocompleteSuggestions */
/** @typedef {import("../tui.js").Component} Component */
/** @typedef {import("../tui.js").Focusable} Focusable */
/** @typedef {import("../tui.js").TUI} TUI */
/** @typedef {import("./select-list.js").SelectListLayoutOptions} SelectListLayoutOptions */
/** @typedef {import("./select-list.js").SelectListTheme} SelectListTheme */
 ...
 * @returns {Iterable<Intl.SegmentData>}
 */
 ...
 *
 * @param {string} line - The text line to wrap
 * @param {number} maxWidth - Maximum visible width per chunk
 * @param {Intl.SegmentData[]} [preSegmented] - Optional pre-segmented graphemes (e.g. with paste-marker awareness).
 *                       When omitted the default Intl.Segmenter is used.
 * @returns {TextChunk[]} Array of chunks with text and position information
 */
...
export class Editor extends RetainedComponent {
	...
	/** @type {SelectList | undefined} */
	autocompleteList;
	/** @type {number} */
	lastAutocompleteStartLine = -1;
	/** @type {number} */
	lastAutocompleteEndLine = -1;
	/** @type {"regular" | "force" | null} */
	...
	/** @param {AutocompleteProvider} provider */
	 ...
	 */
	renderFrame(width) {
		this.lastAutocompleteStartLine = -1;
		this.lastAutocompleteEndLine = -1;
		...
		if (this.autocompleteState && this.autocompleteList) {
			const autocompleteFrame = this.autocompleteList.renderFrame(contentWidth);
			const autocompleteStartLine = result.length;
			this.lastAutocompleteStartLine = autocompleteStartLine;
			for (const line of autocompleteFrame.lines) {
				const lineWidth = visibleWidth(line);
				const linePadding = " ".repeat(Math.max(0, contentWidth - lineWidth));
				result.push(`${leftPadding}${line}${linePadding}${rightPadding}`);
			}
			this.lastAutocompleteEndLine = result.length;
			spans.push(...offsetSpans(autocompleteFrame.spans, autocompleteStartLine, paddingX));
		}

		return { lines: result, spans };
	}
	...
	handleMouseEvent(event) {
		if (!this.autocompleteState || !this.autocompleteList || !event.localPoint) return { consume: false };
		const line = event.localPoint.line;
		if (line < this.lastAutocompleteStartLine || line >= this.lastAutocompleteEndLine) return { consume: false };
		return this.autocompleteList.handleMouseEvent(event);
	}
	...
	handleInput(data) {
		...
		if (this.autocompleteState && this.autocompleteList) {
			...
			if (kb.matches(data, "tui.select.confirm")) {
				const selected = this.autocompleteList.getSelectedItem();
				if (selected && this.applyAutocompleteSelection(selected, { submitSlash: true })) return;
			}
		}
		...
		if (kb.matches(data, "tui.input.submit")) {
			...
			if (this.trySubmitSlashAutocomplete()) return;
	...
	/**
	 * @param {string} data
	 * @param {ReturnType<typeof getKeybindings>} kb
	 * @returns {boolean}
	 */
	...
	/** @returns {boolean} */
	trySubmitSlashAutocomplete() {
		const snapshot = this.getSlashCommandNameCompletionSnapshot();
		if (!snapshot) return false;

		void this.submitSlashAutocomplete(snapshot);
		return true;
	}

	/**
	 * @returns {{ lines: string[], text: string, cursorLine: number, cursorCol: number } | null}
	 */
	getSlashCommandNameCompletionSnapshot() {
		if (!this.autocompleteProvider || !this.isSlashMenuAllowed()) return null;

		const currentLine = this.state.lines[this.state.cursorLine] || "";
		const textBeforeCursor = currentLine.slice(0, this.state.cursorCol);
		if (!textBeforeCursor.startsWith("/") || /\s/.test(textBeforeCursor) || textBeforeCursor.slice(1).includes("/")) {
			return null;
		}
	...
	/**
	 * @param {{ lines: string[], text: string, cursorLine: number, cursorCol: number }} snapshot
	 * @returns {Promise<void>}
	 */
	async submitSlashAutocomplete(snapshot) {
		...
		/** @type {AutocompleteSuggestions | null} */
		...
		try {
			...
			if (controller.signal.aborted || !this.isEditorSnapshotCurrent(snapshot.text, snapshot.cursorLine, snapshot.cursorCol)) {
				return;
			}
			this.submitValue();
			this.tui.requestRender();
		...
		this.applyAutocompleteSuggestions(suggestions, "regular");
		const selected = this.autocompleteList?.getSelectedItem();
		if (selected) this.applyAutocompleteSelection(selected, { submitSlash: true });
		else this.submitValue();
		this.tui.requestRender();
	}

	/**
	 * @param {{ value: string, label: string, description?: string }} selected
	 * @param {{ submitSlash?: boolean }} [options]
	 * @returns {boolean}
	 */
	applyAutocompleteSelection(selected, options = {}) {
		...
		const shouldSubmit = options.submitSlash === true && this.autocompletePrefix.startsWith("/");
		this.cancelAutocomplete();
		if (shouldSubmit) {
	 ...
	 * @param {Array<{ logicalLine: number; startCol: number; length: number }>} visualLines
	 * @param {number} currentVisualLine
	 * @param {number} targetVisualLine
	 */
	 ...
	 *
	 * @param {number} currentVisualCol
	 * @param {number} sourceMaxVisualCol
	 * @param {number} targetMaxVisualCol
	 * @returns {number}
	 */
	 ...
	 *
	 * @param {number} width
	 * @returns {Array<{ logicalLine: number; startCol: number; length: number }>}
	 */
	buildVisualLineMap(width) {
		/** @type {Array<{ logicalLine: number; startCol: number; length: number }>} */
		const visualLines = [];
	...
	createAutocompleteList(prefix, items) {
		...
		list.onSelect = (item) => {
			this.applyAutocompleteSelection(item, { submitSlash: true });
		};
		return list;
	}

	/** @param {boolean} [explicitTab] */
	...
	/** @param {{ force: boolean; explicitTab: boolean }} options */
	...
	isAutocompleteRequestCurrent(requestId, controller, snapshotText, snapshotLine, snapshotCol) {
		return (
			...
			this.isEditorSnapshotCurrent(snapshotText, snapshotLine, snapshotCol)
		);
	}
	 ...
	 * @returns {boolean}
	 */
	isEditorSnapshotCurrent(snapshotText, snapshotLine, snapshotCol) {
		return (
	...
	applyAutocompleteSuggestions(suggestions, state) {
		...
		const bestMatchIndex = this.getBestAutocompleteMatchIndex(suggestions.items, suggestions.matchPrefix ?? suggestions.prefix);
		if (bestMatchIndex >= 0) {

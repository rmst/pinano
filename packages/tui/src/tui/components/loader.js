...
export class Loader extends Text {
	...
	/**
	 * @param {number} width
	 * @returns {import("../render-frame.js").RenderFrame}
	 */
	renderFrame(width) {
		const frame = normalizeRenderFrame(super.renderFrame(width));
		return {
			lines: ["", ...frame.lines],
			spans: offsetSpans(frame.spans, 1),
			sourceSpans: offsetSourceSpans(frame.sourceSpans, 1),
			regions: offsetRegions(frame.regions, 1),
		};
	}

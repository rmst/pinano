...
import { TextSelectionSource, semanticTextFromRenderable } from "../selection-source.js";
...
export class Text extends RetainedComponent {
	...
	/** @type {string[] | undefined} */
	cachedLines;
	/** @type {{ lines: string[], spans: import("../render-frame.js").RenderSpan[], sourceSpans: import("../render-frame.js").RenderSourceSpan[] } | undefined} */
	cachedFrame;
	...
	renderFrame(width) {
		...
		if (!this.text || this.text.trim() === "") {
			...
			this.cachedFrame = { lines: result, spans: [], sourceSpans: [] };
			return this.cachedFrame;
		}
		...
		this.cachedWidth = width;
		this.cachedLines = result;
		const sourceText = semanticTextFromRenderable(this.text);
		this.sourceProvider.setText(sourceText);
		const sourceFrame = renderSegmentedText([
			{
				text: sourceText,
		...
		const sourceSpans = sourceFrameAlignsWithRenderedLines(sourceFrame.lines, result) ? sourceFrame.sourceSpans : [];
		this.cachedFrame = { lines: result.length > 0 ? result : [""], spans: [], sourceSpans };
 ...
 */
function sourceFrameAlignsWithRenderedLines(sourceLines, renderedLines) {
	if (sourceLines.length !== renderedLines.length) return false;
	return sourceLines.every((line, index) => line === semanticTextFromRenderable(renderedLines[index] ?? ""));
}

...
/**
 * @param {string} prefix
 * @returns {{ rawPrefix: string, isAtPrefix: boolean, isQuotedPrefix: boolean }}
 */
...
/**
 * @param {string} path
 * @param {{ isDirectory: boolean, isAtPrefix: boolean, isQuotedPrefix: boolean }} options
 * @returns {string}
 */
 ...
 * @returns {Promise<Array<{ path: string, isDirectory: boolean }>>}
 */
async function walkDirectoryWithFd(baseDir, fdPath, query, maxResults, signal) {
	...
	return await new Promise((resolve) => {
		...
		/** @param {Array<{ path: string, isDirectory: boolean }>} results */
		...
		child.on("close", (code) => {
			...
			/** @type {Array<{ path: string, isDirectory: boolean }>} */
			const results = [];
 ...
 * @property {(argumentPrefix: string) => Awaitable<AutocompleteItem[] | null>} [getArgumentCompletions] Function to get argument completions for this command. Returns null if no argument completion is available
 */

/**
 * @typedef {object} AutocompleteSuggestions
 * @property {AutocompleteItem[]} items
 * @property {string} prefix The text range to replace when applying a completion (e.g., "/he" or "src/")
 * @property {string} [matchPrefix] The query to use when choosing the best initially selected item. Defaults to prefix.
 */

/**
 * @typedef {object} AutocompleteProvider
 * @property {(lines: string[], cursorLine: number, cursorCol: number, options: { signal: AbortSignal, force?: boolean }) => Promise<AutocompleteSuggestions | null>} getSuggestions Get autocomplete suggestions for current text/cursor position. Returns null if no suggestions available
 * @property {(lines: string[], cursorLine: number, cursorCol: number, item: AutocompleteItem, prefix: string) => { lines: string[], cursorLine: number, cursorCol: number }} applyCompletion Apply the selected item. Returns the new text and cursor position
 * @property {(lines: string[], cursorLine: number, cursorCol: number) => boolean} [shouldTriggerFileCompletion] Check if file completion should trigger for explicit Tab completion
 */
...
export class CombinedAutocompleteProvider {
	 ...
	 * @returns {{ baseDir: string, query: string, displayBase: string } | null}
	 */
	...
	/**
	 * @param {string} query
	 * @param {{ isQuotedPrefix: boolean, signal: AbortSignal }} options

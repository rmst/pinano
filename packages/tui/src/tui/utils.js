import "./utils-vendor/intl-segmenter-polyfill.js";
import { eastAsianWidth } from "./utils-vendor/east-asian-width.js";
...
const TERMINAL_CONTROL_REGEX = /[\x00-\x1f\x7f-\x9f]/;
...
function graphemeWidth(segment) {
	...
	if (segment.length > 1) {
		for (const char of segment.slice(1)) {
			const c = /** @type {number} */ (char.codePointAt(0));
...
export function visibleWidth(str) {
	...
	const sanitized = sanitizeRenderableText(str);
	...
	// Check cache
	const cached = widthCache.get(sanitized);
	if (cached !== undefined) {
		return cached;
	}

	// Strip display-only escape sequences before measuring visible text.
	let clean = sanitized;
	...
	widthCache.set(sanitized, width);
 ...
 * @returns {{ code: string, length: number }}
 */
function extractCsiCode(str, pos, bodyStart) {
	let j = bodyStart;
	while (j < str.length) {
		if (isCsiFinalByte(str.charCodeAt(j))) {
			return { code: str.substring(pos, j + 1), length: j + 1 - pos };
		}
		j++;
	}
	return { code: str.substring(pos), length: str.length - pos };
}
 ...
 * @returns {{ code: string, length: number }}
 */
function extractStringControlCode(str, pos, bodyStart) {
	let j = bodyStart;
	while (j < str.length) {
		if (str[j] === "\x07" || str.charCodeAt(j) === 0x9c) {
			return { code: str.substring(pos, j + 1), length: j + 1 - pos };
		}
...
/**
 * Extract a terminal control sequence from a string at the given position.
 * This recognizes ECMA-48 CSI/string controls broadly so unsupported terminal
 * commands are treated as one zero-width unit by layout code and can be stripped
 * cleanly by sanitizeRenderableText.
 *
 * @param {string} str
 * @param {number} pos
...
export function extractAnsiCode(str, pos) {
	...
	if (code === 0x9b) return extractCsiCode(str, pos, pos + 1);
	if (code === 0x90 || code === 0x98 || code === 0x9d || code === 0x9e || code === 0x9f) {
		return extractStringControlCode(str, pos, pos + 1);
	}
	if (code >= 0x80 && code <= 0x9f) {
		return { code: str[pos], length: 1 };
	}
	if (str[pos] !== "\x1b") return null;

	const next = str[pos + 1];
	if (next === undefined) return { code: str[pos], length: 1 };
	if (next === "[") return extractCsiCode(str, pos, pos + 2);
	if (next === "]" || next === "P" || next === "_" || next === "^" || next === "X") {
		return extractStringControlCode(str, pos, pos + 2);
	}

	let j = pos + 1;
	while (j < str.length) {
		const nextCode = str.charCodeAt(j);
		if (isEscapeFinalByte(nextCode)) {
			return { code: str.substring(pos, j + 1), length: j + 1 - pos };
		}
		j++;
	}
...
/** @typedef {"\x07" | "\x1b\\"} Osc8Terminator */

/**
 * @typedef {object} ActiveHyperlink
 * @property {string} params
 * @property {string} url
 * @property {Osc8Terminator} terminator
 */

/**
 * @param {string} ansiCode
 * @returns {ActiveHyperlink | null | undefined}
 */
function parseOsc8Hyperlink(ansiCode) {
	...
	/** @type {Osc8Terminator} */
	let terminator;
	if (ansiCode.endsWith("\x07")) {
		terminator = "\x07";
	} else if (ansiCode.endsWith("\x1b\\")) {
		terminator = "\x1b\\";
	} else {
		return undefined;
	}
...
/**
 * @param {ActiveHyperlink | null} hyperlink
 * @returns {boolean}
 */
function isSafeOsc8Hyperlink(hyperlink) {
	if (hyperlink === null) return true;
	return !TERMINAL_CONTROL_REGEX.test(hyperlink.params) && !TERMINAL_CONTROL_REGEX.test(hyperlink.url);
}
 ...
 * @returns {string}
 */
function renderableTerminalControl(code) {
	if (SGR_SEQUENCE_REGEX.test(code)) return code;
	const hyperlink = parseOsc8Hyperlink(code);
	if (hyperlink !== undefined && isSafeOsc8Hyperlink(hyperlink)) return code;
	return "";
}
...
export function sanitizeRenderableText(text, options = {}) {
	if (!TERMINAL_CONTROL_REGEX.test(text)) return text;
	...
	while (i < text.length) {
		...
		if (ch === "\x1b" || (code >= 0x80 && code <= 0x9f)) {
			const control = extractAnsiCode(text, i);
			if (control) {
				out += renderableTerminalControl(control.code);
				i += control.length;
			} else {
				i++;
			}
		...
		if (ch === "\n") {
			if (preserveLineBreaks) out += "\n";
			i++;
			continue;
		}

		if (ch === "\r" || code < 0x20 || code === 0x7f || (code >= 0x80 && code <= 0x9f)) {
			i++;
			continue;
		}
...
/**
 * @param {ActiveHyperlink} hyperlink
 * @returns {string}
 */
...
/**
 * @param {Osc8Terminator} terminator
 * @returns {string}
 */
 ...
 * @param {number} width - Maximum visible width per line
 * @returns {string[]} Array of wrapped lines (NOT padded to width)
 */
export function wrapTextWithAnsi(text, width) {
	text = sanitizeRenderableText(text, { preserveLineBreaks: true });
...
function breakLongWord(word, width, tracker) {
	...
	let i = 0;
	/** @type {Array<{ type: "ansi" | "grapheme", value: string }>} */
	const segments = [];
 ...
 *
 * @param {string} line - Line of text (may contain ANSI codes)
 * @param {number} width - Total width to pad to
 * @param {(text: string) => string} bgFn - Background color function
 * @returns {string} Line with background applied and padded to width
 */
 ...
 * @param {string} [ellipsis] - Ellipsis string to append when truncating (default: "...")
 * @param {boolean} [pad] - If true, pad result with spaces to exactly maxWidth (default: false)
 * @returns {string} Truncated text, optionally padded to exactly maxWidth
 */
export function truncateToWidth(
	...
	text = sanitizeRenderableText(text);
	ellipsis = sanitizeRenderableText(ellipsis);
 ...
 * @param {number} length
 * @param {boolean} [strict] - If true, exclude wide chars at boundary that would extend past the range
 * @returns {string}
 */
...
export function sliceWithWidth(
	...
	line = sanitizeRenderableText(line);
 ...
 * @param {number} afterLen
 * @param {boolean} [strictAfter]
 * @returns {{ before: string, beforeWidth: number, after: string, afterWidth: number }}
 */

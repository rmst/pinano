...
import { promptContentWithImages } from "../../../protocol/src/prompt-images.js"
...
const MODEL_VISIBLE_ROLES = new Set(["system", "developer", "user", "assistant", "toolResult"])

function defaultConvertToLlm(messages) {
	return messages.filter((m) => MODEL_VISIBLE_ROLES.has(m.role))
}
...
class PendingMessageQueue {
	...
	/** @param {(message: AgentMessage) => boolean} predicate */
	takeWhere(predicate) {
		const removed = []
		const kept = []
		for (const message of this.messages) {
			if (predicate(message)) removed.push(message)
...
export class Agent {
	...
	/** @param {(item: { behavior: "steer" | "followUp", message: AgentMessage }) => boolean} predicate */
	takeQueuedMessages(predicate) {
		return [
			...this.steeringQueue.takeWhere((message) => predicate({ behavior: "steer", message })).map((message) => ({ behavior: "steer", message })),
			...this.followUpQueue.takeWhere((message) => predicate({ behavior: "followUp", message })).map((message) => ({ behavior: "followUp", message })),
		]
	}

	/** @param {{ behavior: "steer" | "followUp", message: AgentMessage }[]} items */
	prependQueuedMessages(items) {
		const steering = items.filter((item) => item.behavior === "steer").map((item) => item.message)
		const followUps = items.filter((item) => item.behavior === "followUp").map((item) => item.message)
		if (steering.length > 0) this.steeringQueue.prepend(steering)
		if (followUps.length > 0) this.followUpQueue.prepend(followUps)
	}
	...
	createSidecarAgent(options) {
		const sidecar = new Agent({
			...
			preTurnMessages: undefined,
			projectMessagesForNextAction: undefined,
	...
	createLoopConfig(options = {}) {
		...
		return {
			...
			getApiKey: this.getApiKey,
			getPreTurnMessages: async (ctx) => this.preTurnMessages?.(ctx) ?? [],
			projectMessagesForNextAction: async (ctx) => {
				const messages = await this.projectMessagesForNextAction?.(ctx)
				if (messages) this._state.messages = messages.slice()
				return messages
			},
			getSteeringMessages: async () => {

...
/**
 * @typedef {"files" | "search" | "workbench" | "source-control" | "pinano" | "terminal"} ProjectPane
...
const PROJECT_PANES = new Set(["files", "search", "workbench", "source-control", "pinano", "terminal"])
...
/**
 * @param {PinanoRoute | undefined | null} route
...
/**
 * @param {PinanoRoute | undefined} route
 * @returns {string | undefined}
 */
...
function projectPathForRoute(route, project) {
	...
	const pane = project.pane ?? (project.file ? "workbench" : route.type === "session" ? "pinano" : undefined)
...
function parseProjectRoute(url) {
	...
	return sessionId
		? sessionRoute(sessionId, root, root, { ...project, pane: project?.pane ?? "pinano" })

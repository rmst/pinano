...
import { PINANO_SESSION_ENV } from "../../../protocol/src/internal-api-env.js"
import { createSeatbeltSandboxArgs } from "./seatbelt-sandbox.js"
import { environmentContainerHomePath, environmentHomePath, managedContainerHomePath, optionalPinanoHomePath, optionalRuntimeSourceReferencePath } from "./paths.js"
...
import { addImplicitPinanoStateMounts, addManagedContainerHomeMount, normalizePinanoStateMount } from "./tool-state-mounts.js"
...
export const terminalWorkerPath = join(here, "terminal-worker.js")
const defaultWorkerEntry = "packages/server/src/app/tool-worker.js"
...
const sourceSnapshotVersion = "pinano-source-v3"
const sourcePackageRoots = ["packages/protocol", "packages/server"]
...
function sessionWorkspaceEnv(sessionDir, sessionId = undefined) {
	const session = sessionId ? { [PINANO_SESSION_ENV]: sessionId } : {}
	if (!sessionDir) return session
...
function serviceHomePath(env = process.env) {
	...
	if (!home || !isAbsolute(home)) throw new Error("Native sandbox isolatedHome false requires the Pinano service HOME to be an absolute path")
	return resolve(home)
}
...
function runtimeProbeFor(runtime) {
	if (requiredRuntimeName(runtime) === "qn") return [
		...
		"  qn --version 2>/dev/null || true",
		"else",
		"  echo 'pinano worker requires qn' >&2",
		"  exit 127",
		"fi",
	].join("\n")
	return runtimeProbe
...
export class LocalWorkerLauncher {
	/** @param {{ cwd?: string, sessionDir?: string, sessionId?: string, workerPath?: string, runtime?: "qn" }} options */
	async start(options) {
		await prepareSessionDir(options.sessionDir)
		const env = localWorkerEnv({ platform: process.platform, sessionDir: options.sessionDir, sessionId: options.sessionId })
...
export class NativeSandboxWorkerLauncher {
	/** @param {{ platform?: string, mountPaths?: any[], useSessionWd?: boolean, isolatedHome?: boolean, pinanoStateMount?: unknown, sandboxExecCommand?: string, bwrapCommand?: string }} [options] */
	constructor(options = {}) {
	...
	async start(options) {
		...
		const env = localWorkerEnv({ platform: this.platform, ...(this.isolatedHome ? { toolHome: home } : { home }), sessionDir: workerSessionDir, sessionId: options.sessionId })
		...
		if (this.platform === "darwin") {
			...
			else await assertDirectory(home, "Native sandbox HOME")
			...
			args = createSeatbeltSandboxArgs({
				...
				writableRoots: absolutePathVariantSet([...writableRoots, ...(this.isolatedHome ? [home] : []), ...macosSeatbeltTempRoots()]),
			})
			...
			args = bubblewrapArgs({
				workdir: launchCwd,
				roots: writableRoots,
				writableRoots: this.isolatedHome ? [home] : [],
...
function managedContainerOutputRedactions({ workdir, mounts, env, labels, extraArgs, command }) {
	...
	return [
		...
		...replacementValues([`com.pinano.mounts=${mountLabel(mounts)}`], "[redacted mount label]"),
...
function managedContainerHomeProbeScript(env) {
	...
	const marker = `${env.HOME}/.pinano-home-write-test`
	return [
...
async function assertManagedContainerHomeWritable(engine, container, env) {
	...
	try {
		...
		throw new Error(`Managed container isolated HOME is not writable at ${env.HOME}. Check sandbox.extraArgs user settings or set isolatedHome to false for this environment.`, { cause: err })
	}
}
...
async function startManagedContainer({ engine, image, workdir, mounts, env = {}, network, extraArgs = [], namePrefix = "pinano-worker", labels = [], publish = [], command = undefined, description = "Managed container", failureFooter = undefined }) {
	...
	const args = [
		...
		"--label", `com.pinano.mounts=${mountLabel(runMounts)}`,
...
function managedPreviewEnv(options) {
	return {
		...
		...sessionWorkspaceEnv(options.sessionDir, options.sessionId),
		PINANO_PREVIEW: "1",
		PINANO_PREVIEW_ID: options.id,
		PINANO_PREVIEW_NAME: options.name,
		PINANO_HOST: options.bindHost,
		PINANO_PORT: String(options.bindPort),
		PINANO_PUBLIC_URL: options.publicUrl,
		...(options.logPath ? { PINANO_PREVIEW_LOG: options.logPath } : {}),
	}
}

function managedPreviewCommand(command, options = {}) {
	const quoted = shellQuote(command)
	const marker = shellQuote(`[pinano] --- ${options.appendLog ? "restarting" : "starting"} ${new Date().toISOString()} ---`)
	const redirect = options.appendLog ? ">>" : ">"
	return [
		`if [ -n "\${PINANO_PREVIEW_LOG:-}" ] && mkdir -p "$(dirname "$PINANO_PREVIEW_LOG")"; then`,
		`printf '%s\\n' ${marker} ${redirect} "$PINANO_PREVIEW_LOG"`,
		`printf '%s\\n' '[pinano] preview process starting' >> "$PINANO_PREVIEW_LOG"`,
		`exec sh -lc ${quoted} >> "$PINANO_PREVIEW_LOG" 2>&1`,
		"fi",
		`exec sh -lc ${quoted}`,
	].join("\n")
}
...
function managedPreviewLabels(options) {
	return [
		"com.pinano.preview=true",
		`com.pinano.preview.id=${options.id}`,
		...(options.sessionId ? [`com.pinano.session=${options.sessionId}`] : []),
		...(options.environmentId ? [`com.pinano.environment=${options.environmentId}`] : []),
	]
}
...
class ContainerExecWorkerLauncher {
	...
	async start(options) {
		...
		try {
			...
			const env = { ...this.env, ...sessionWorkspaceEnv(workerSessionDir, options.sessionId) }
...
export class ManagedContainerPreviewProcess {
	...
	constructor(options) {
		...
		this.logPath = previewLogPath(this.sessionDir, this.name)
		this.environmentId = options.environmentId
		this.sandbox = sandbox
		this.env = options.env
		this.isolatedHome = sandbox.isolatedHome !== false
		this.pinanoStateMount = options.pinanoStateMount
	...
	async start() {
		...
		const mounts = await addToolStateMounts(effectiveSandboxMounts({
		...
		const containerLogPath = previewLogPath(previewSessionDir, this.name)
...
export class ManagedContainerWorkerLauncher {
	/** @param {{ engine?: string, image?: string, remoteRoot?: string, mountPaths?: any[], useSessionWd?: boolean, isolatedHome?: boolean, pinanoStateMount?: unknown, env?: Record<string, string>, network?: string, extraArgs?: string[], createSourceSnapshot?: typeof createSourceSnapshot }} [options] */
	constructor(options = {}) {
		this.engine = options.engine
	...
	async start(options) {
		...
		const env = { ...this.env, ...home.env, ...sessionWorkspaceEnv(workerSessionDir, options.sessionId) }
...
export function createWorkerLauncher(environment) {
	...
	if (sandbox.type === "native") return new NativeSandboxWorkerLauncher({ mountPaths: sandbox.mountPaths, useSessionWd: sandbox.useSessionWd, isolatedHome: sandbox.isolatedHome, pinanoStateMount: environment.pinanoStateMount })

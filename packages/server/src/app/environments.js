...
function normalizeSandbox(id, value, target) {
	...
	assertOnlyFields(sandbox, ["type", "image", "container", "engine", "useSessionWd", "isolatedHome", "mountPaths", "env", "network", "extraArgs"], `Environment ${id} sandbox`)
	...
	if (container && isolatedHome !== undefined) throw new TypeError(`Environment ${id} sandbox isolatedHome is only valid for native or Pinano-managed container sandboxes`)

...
//                  handoff summary (`compactionSummary: true`)
...
import { requestRemoteCompaction, supportsRemoteCompaction } from "./remote.js"
	...
	if (agent.persistCompaction) {
		try {
			const newEntryId = await agent.persistCompaction({
				...
				displayMessage,
				replacementContext: { kind: "messages", messages: replacementMessages },
			})
...
export function findCompactionCutIndex(messages, keepLast) {
	...
	while (cut < messages.length && (
		...
		|| messages[cut]?.compactionMemento

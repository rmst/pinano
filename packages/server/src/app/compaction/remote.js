...
// normal Cerex compaction persistence path install that replacement history.
...
import { isFastModeEligibleModel } from "../agent/fast-mode.js"
import { resolveModelStreamOptions } from "../model/auth.js"
import { modelCompactionHandoffMessage } from "./summary.js"
...
function isGeneratedContextMessage(message) {
	return isProjectContextMessage(message) || message?.automated === true || message?.maintenance === true
}
...
function truncateMessage(message, maxTokens) {
	if (typeof message.content === "string") {
		...
		return content ? { ...message, content, remoteCompactionRetained: true } : undefined
	}
	...
	return content.length > 0 ? { ...message, content, remoteCompactionRetained: true } : undefined
}
...
export function selectRemoteCompactionRetainedMessages(messages, maxTokens = REMOTE_COMPACTION_RETAINED_TOKEN_BUDGET) {
	...
	for (const message of [...messages].reverse()) {
		...
		if (tokens <= remaining) {
			retained.push({ ...message, remoteCompactionRetained: true })
			remaining -= tokens
			continue
		}
...
function buildRemoteCompactionMessages(agent, messages) {
	...
	return agent.streamFn?.serviceMediated
		? modelMessages
		: prependEnvironmentContext(agent.environmentContext?.(), modelMessages)
}

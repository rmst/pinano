...
export async function staticPreviewDefinitionForRecord(workspace, record, name = undefined, options = {}) {
	...
	if (record.scopeKind === PREVIEW_ROOT_KIND_SOURCE) {
		if (!record.rootPath?.endsWith(PREVIEW_FILE_SUFFIX)) return undefined
		let definition
		try {
		...
		return isStaticPreviewDefinition(definition) && (!name || definition.name === name) ? definition : undefined
	}

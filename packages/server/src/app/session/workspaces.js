...
const projectLocalWorktreeMarkers = ["/.cerex/wt/", "/.pinano/wt/"]
...
function projectLocalWorktreeRepoRoot(path) {
	const absolute = resolve(path)
	const index = projectLocalWorktreeMarkers
		.map((marker) => absolute.indexOf(marker))
...
export function branchNoticeMessageText(report) {
	const lines = [
		...
		"",
		"Use the fresh session directory for durable session files:",
		report.targetDir,
		"",
		"Files from the parent session directory were not copied. Do not continue editing files under the old session directory:",
		report.sourceDir,
		"",

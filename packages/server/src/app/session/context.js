...
import { SESSION_WORKTREE_LIFECYCLE_MAINTENANCE, getEffectiveSessionProperties } from "./properties.js"
...
function pruneBeforeLatestResponsesCompaction(entries) {
	...
	return entries.filter((entry, index) => index >= latest || entry.message?.remoteCompactionRetained === true)
}
...
export function projectMaintenanceMessage(message, options = {}) {
	if (!message?.automated && !message?.maintenance) {
		options.maintenanceKind = undefined
		return message
	}
	if (message.role === "user") {
		options.maintenanceKind = typeof message.maintenance === "string" ? message.maintenance : undefined
		return undefined
	}
	const maintenanceKind = typeof message.maintenance === "string" ? message.maintenance : options.maintenanceKind
...
function stripStaleAutomatedMessages(messages) {
	...
	if (last?.automated || last?.maintenance) return messages

...
import { dataRoot, isPinanoTestProcess } from "./paths.js"
import { buildBundledBubblewrapFromSource, verifiedCachedSourceBuiltBubblewrapPath } from "./bwrap-source-build.js"
...
export const bundledBubblewrapMirrorRepository = "https://github.com/rmst/pinano"
...
function configuredDownloadTimeoutMs(options = {}) {
	...
	const envValue = Number(process.env.PINANO_BUNDLED_BWRAP_DOWNLOAD_TIMEOUT_MS)
...
export async function bundledBubblewrapBuildFromSourceEnabled(options = {}) {
	...
	if (isPinanoTestProcess() && !process.env.PINANO_HOME) return false
	const { loadSettings } = await import("./settings.js")
	try {
		return (await loadSettings()).buildFromSource === true
	} catch (err) {
		if (isPinanoTestProcess() && /Pinano tests refuse to use persistent PINANO_HOME|Pinano tests require an isolated PINANO_HOME/.test(err?.message ?? String(err))) return false
		throw err
	}
}
...
function autoInstallDisabled(options = {}) {
	...
	if (process.env.PINANO_BUNDLED_BWRAP_AUTO_INSTALL === "0") return true
	return isPinanoTestProcess() && options.allowInTests !== true
}

...
function shellFilePath(token, cwd, includeUnverifiedPaths) {
	const abs = isAbsolute(token) ? token : resolve(cwd, token)
	if (includeUnverifiedPaths) return abs
	try {
 ...
 */
export function extractShellCommandPathInfo(command, cwd, options = {}) {
	/** @type {string[]} */
	const paths = []
	/** @type {string[]} */
	...
	for (const segment of commandSegments(command)) {
		...
		const segmentPaths = hasOutputRedirection(segment.tokens)
			? []
			: extracted.tokens
				.map((token) => shellFilePath(token, cwd, options.includeUnverifiedPaths === true))

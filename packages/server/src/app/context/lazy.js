...
import { loadContextFilesForPath } from "./files.js"
import { LAZY_NOTICE_HEADING, PROJECT_CONTEXT_HEADING } from "./format.js"
import { extractShellCommandPathInfo } from "./shell-paths.js"

/** @typedef {import("./files.js").ContextFile} ContextFile */
...
export class LazyContextLoader {
	...
	/** @type {Set<string>} */
	loaded
	/** @type {(cwd: string, path: string, options?: { requireFile?: boolean }) => ContextFile[] | Promise<ContextFile[]>} */
	loadContextFiles

	/** @param {{ cwd: string, alreadyLoaded: Iterable<string>, loadContextFiles?: (cwd: string, path: string, options?: { requireFile?: boolean }) => ContextFile[] | Promise<ContextFile[]> }} options */
	constructor(options) {
		this.cwd = resolve(options.cwd)
		this.loaded = new Set()
		this.loadContextFiles = options.loadContextFiles ?? ((cwd, path, loadOptions = {}) => loadContextFilesForPath({ cwd, path, ...loadOptions }))
	 ...
	 * @returns {ContextFile[]}
	 */
	loadForPath(path, options = {}) {
		const files = this.loadContextFiles(this.cwd, path, options)
		if (files && typeof files.then === "function") throw new Error("Async context loaders require loadForPathAsync")
		return this.filterNewFiles(files)
	}

	/** @param {string} path @param {{ requireFile?: boolean }} [options] @returns {Promise<ContextFile[]>} */
	async loadForPathAsync(path, options = {}) {
		return this.filterNewFiles(await this.loadContextFiles(this.cwd, path, options))
	}

	/** @param {ContextFile[]} files @returns {ContextFile[]} */
	filterNewFiles(files) {
		const out = []
...
function extractExecCommandPaths(args, cwd, options) {
	...
	const extracted = extractShellCommandPathInfo(command, baseCwd, options)
 ...
 * @param {string} cwd
 * @param {any} [result]
 * @param {{ includeUnverifiedPaths?: boolean }} [options]
 * @returns {{ paths: string[], manuallyLoadedContextPaths: string[] }}
 */
export function extractToolPaths(toolName, args, cwd = process.cwd(), result = undefined, options = {}) {
	...
	if (toolName === "exec_command") return extractExecCommandPaths(args, cwd, options)
	...
	if (toolName === "grep" && args && typeof args.path === "string") {
		return { paths: options.includeUnverifiedPaths === true || isRegularFile(resolvePathForCwd(args.path, cwd)) ? [args.path] : [], manuallyLoadedContextPaths: [] }
	}

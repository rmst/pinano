import { estimateContextUse } from "./summary.js"
...
/**
 * @param {import("./accounting.js").PerMessageEntry} entry
 * @returns {string}
 */
...
/**
 * @param {import("./accounting.js").ContextBreakdown} breakdown
...
export function formatContextReport({ messages = [], systemPrompt = "", tools = [], model } = {}) {
	...
	const lines = [
		...
		`  last reported baseline:         ${usage.reportedTokens > 0 ? formatTokens(usage.reportedTokens) + " tok" : "—"}`,
		`  unreported local delta:         ${usage.reportedTokens > 0 ? formatTokens(usage.reportedDeltaTokens) + " tok" : "—"}`,
		"",
		"Breakdown",

...
function inspectDiagnostics(diagnostics) {
	return diagnostics?.inspect?.() ?? diagnostics?.status?.()
}
...
async function collectInspect(options, requestUrl) {
	...
	if (sections.has("diagnostics")) result.diagnostics = inspectDiagnostics(options.getDiagnostics?.())

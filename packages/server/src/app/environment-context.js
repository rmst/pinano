...
import { environmentContainerHomePath as defaultEnvironmentContainerHomePath, environmentHomePath as defaultEnvironmentHomePath, managedContainerHomePath, optionalPinanoHomePath, optionalRuntimeSourceReferencePath } from "./paths.js"
...
import { addImplicitPinanoStateMounts, addManagedContainerHomeMount, normalizePinanoStateMount, pinanoStateMountEnabled } from "./tool-state-mounts.js"
...
function effectiveMountsForContext(environment, initialCwd, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts = [], environmentContainerHomePath = defaultEnvironmentContainerHomePath) {
...
function managedContainerHomeLine(environment, environmentContainerHomePath) {
	...
	if (environment?.sandbox?.isolatedHome === false) {
		...
		return configuredHome
			? `HOME inside this environment: ${xmlText(configuredHome)} (configured by sandbox.env; not isolated by Pinano).`
			: "HOME inside this environment: image/container default (not isolated by Pinano)."
	}
	try {
		return `HOME inside this environment: ${xmlText(managedContainerHomePath)} (Pinano-managed container path backed by host path ${xmlText(environmentContainerHomePath(environment.id))}).`
	} catch {
		return `HOME inside this environment: ${xmlText(managedContainerHomePath)} (Pinano-managed container path).`
	}
}

function containerFilesystemLines(environment, initialCwd, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts = [], environmentContainerHomePath = defaultEnvironmentContainerHomePath) {
	const sandbox = environment.sandbox
	const mounts = effectiveMountsForContext(environment, initialCwd, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts, environmentContainerHomePath)
...
function nativeHomeLine(environment, environmentHomePath, serviceHome) {
	if (environment?.sandbox?.isolatedHome === false) {
		return `HOME inside this environment: ${xmlText(serviceHome)} (same as the Pinano service HOME; not isolated by Pinano).`
	}
	try {
...
function nativeFilesystemLines(environment, initialCwd, platform, environmentHomePath, serviceHome, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts = []) {
	const mounts = effectiveMountsForContext(environment, initialCwd, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts)
...
function filesystemLines(environment, initialCwd, platform, environmentHomePath, environmentContainerHomePath, serviceHome, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts = []) {
	const sandbox = environment?.sandbox
	...
	if (sandbox.type === "native") return nativeFilesystemLines(environment, initialCwd, platform, environmentHomePath, serviceHome, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts)
	if (sandbox.type === "container") return containerFilesystemLines(environment, initialCwd, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts, environmentContainerHomePath)
	return [`Filesystem: tool sandbox is ${sandbox.type}.`]
}

function environmentBody(environment, initialCwd, platform, arch, environmentHomePath, environmentContainerHomePath, serviceHome, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts = []) {
	return [
		`<environment ${environmentAttrs(environment)}>`,
		`Runs: ${runsDescription(environment, platform, arch)}.`,
		...filesystemLines(environment, initialCwd, platform, environmentHomePath, environmentContainerHomePath, serviceHome, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts),
		`</environment>`,
	].filter(Boolean)
}

export function sessionWorkspaceToolPathForEnvironment(environment, initialCwd, runtimeSourceReferencePath, workspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts = [], environmentContainerHomePath = defaultEnvironmentContainerHomePath) {
	...
	const mounts = effectiveMountsForContext(environment, initialCwd, runtimeSourceReferencePath, workspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts, environmentContainerHomePath)
...
function sessionWorkspaceLines(environment, initialCwd, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts = [], sessionId = undefined, environmentContainerHomePath = defaultEnvironmentContainerHomePath) {
	const path = sessionWorkspaceToolPathForEnvironment(environment, initialCwd, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts, environmentContainerHomePath)
	if (!path && !sessionId) return []
	return [
		"Session workspace:",
		...(sessionId ? [`- PINANO_SESSION: ${xmlText(sessionId)}`] : []),
		...(path ? [
...
export function environmentContextFor(options = {}) {
	...
	const lines = [
		...
		"Switch environments or cwd with `pinano session set cwd <path>` or `pinano session set environment <id>`.",
		...sessionWorkspaceLines(currentEnvironment, initialCwd, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts, sessionId, environmentContainerHomePath),
		...sessionPreviewLines(previewPublicUrl),
		...environments.flatMap((environment) => environmentBody(environment, initialCwd, platform, arch, environmentHomePath, environmentContainerHomePath, hostUserHome, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts)),
		"</environment_context>",
	].filter(Boolean)
	return lines.join("\n")
}

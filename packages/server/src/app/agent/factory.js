...
import { makeAutoCompactTransform } from "../compaction/auto.js"
import { resolveModelStreamOptions } from "../model/auth.js"
import { LazyContextLoader, formatLazyContextNotice, extractToolPaths } from "../context/lazy.js"
import { activeContextFiles } from "../session/context.js"
import { recordFileCheckpoint } from "../session/file-checkpoints.js"
import { createExecutorProxyTools } from "../workers/tool/executor-tools.js"
import { modelInstructionsForContext, toolProfileForModel } from "../model/instructions/index.js"
...
export function createAgentRuntime(options) {
	const lazyContext = new LazyContextLoader({
		...
		...(options.workspace ? { loadContextFiles: (cwd, path, loadOptions) => options.workspace.context.loadForPath(cwd, path, loadOptions) } : {}),
	})
	/** @type {Agent} */
	let agent
	const codeModeRuntime = new CodeModeRuntime({
		...
		recordPlanUpdate: async (update, source) => {
			if (!agent?.codeModeApiRequest) return
			await agent.codeModeApiRequest({ op: "plan.update", update, source })
		},
	})
	...
	const beforeFileMutation = async (/** @type {{ absolutePath: string }} */ info) => {
		...
		return recordFileCheckpoint(agent?.session, info.absolutePath, options.workspace)
	}
	const baseToolsForModel = (/** @type {any} */ model) => {
		const toolProfile = toolProfileForModel(model)
		return options.toolExecutor
			? createExecutorProxyTools(options.cwd, options.toolExecutor, { getScope: () => agent?.codeModeApiScopeForTool?.(), toolProfile })
	...
	agent = new Agent({
		...
		afterToolCall: async (ctx) => {
			...
			const extracted = extractToolPaths(ctx.toolCall.name, ctx.args, cwd, ctx.result, { includeUnverifiedPaths: !!options.workspace })
			lazyContext.markLoaded(extracted.manuallyLoadedContextPaths)
			const newFiles = []
			const requireFile = ctx.toolCall.name === "grep" || ctx.toolCall.name === "exec_command"
			for (const path of extracted.paths) newFiles.push(...await lazyContext.loadForPathAsync(path, { requireFile }))
	...
	agent.environmentContext = initialEnvironmentContext
	agent.skillsContext = undefined
...
/**
 * Create a Cerex sidecar Agent with model-visible configuration copied from a
 ...
 * sync with ordinary Cerex agents without inheriting visible-run lifecycle
 * state.
 * @param {object} options
 * @param {string} options.cwd
 ...
 */
export function createSidecarAgentRuntime(options) {
	const base = options.baseAgent
	const sidecar = createAgentRuntime({
		cwd: options.cwd,
	...
	sidecar.environmentContext = base.environmentContext
	...
	sidecar.transformContext = withModelContextTransforms(
		() => sidecar.environmentContext?.(),
		() => sidecar.skillsContext?.(),

...
// Launchers own the base PINANO_PROCESS_TITLE. Role defaults are prefixes so truncated process-name views still show the role; env vars such as PINANO_PROCESS_TITLE_SERVICE_SUFFIX can override this without changing how services are spawned.
function roleEnvKey(role, part) {
	return `PINANO_PROCESS_TITLE_${role.replace(/[^a-z0-9]/gi, "_").toUpperCase()}_${part}`
}
...
export function processTitleForRole(role, env = process.env) {
	const base = env.PINANO_PROCESS_TITLE
	if (!base) return ""

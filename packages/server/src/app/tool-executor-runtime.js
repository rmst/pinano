...
import { loadEnvironmentRegistry, resolveExecutionEnvironment, resolveSessionWd } from "./environments.js"
...
import { ManagedContainerPreviewProcess, createWorkerLauncher } from "./worker-launchers.js"
...
export class ToolExecutorRuntime {
	...
	resolveTargetFor(session, props, registry, environmentId = undefined) {
		...
		return { ...target, sandbox, sessionWd, sessionDir, sessionId, pinanoStateMount, workerScope: sessionId ? `session:${sessionId}` : "runtime" }
	}
	...
	workerKey(target) {
		return `${target.workerScope}\0${target.environmentId}\0${JSON.stringify(target.target)}\0${JSON.stringify(target.sandbox)}\0${target.pinanoStateMount || false}`
	}
	...
	scheduleWorkerIdleCheck(key, worker, delayMs = undefined) {
		...
		const timer = setTimeout(() => {
			...
			this.pruneIdleWorker(key, worker).catch((err) => {
				process.stderr.write(`[pinano-tool-worker:${worker.environmentId}] idle prune failed: ${err?.stack ?? err}\n`)
			})
	...
	assertPreviewTargetSupported(target) {
		...
		if (target.sandbox?.type === "container" && target.sandbox?.container) {
			throw Object.assign(new Error("Session previews require a Pinano-managed container, local, or native worker environment"), { status: 501 })
		}
	}
	...
	async startManagedContainerPreview(id, params, target) {
		...
		const preview = await this.managedContainerPreviewFactory({
			...
			environmentId: target.environmentId,
			sandbox: target.sandbox,
			pinanoStateMount: target.pinanoStateMount,
		})

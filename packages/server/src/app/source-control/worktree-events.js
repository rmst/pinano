...
import { LEGACY_PRODUCT_STATE_DIRECTORY, PRODUCT_STATE_DIRECTORY } from "../../../../protocol/src/product.js"

import { effectiveSandboxMounts, hostPathForMountedPath } from "../sandbox/paths.js"
import { addSessionWorkspaceRootMount } from "../workers/tool/state-mounts.js"
...
export const WORKTREE_EVENT_ROUTE = "/internal/worktrees/events"
export const INTEGRATION_TARGET_CONFIG = "cerex-integration-target"
export const INITIAL_HEAD_CONFIG = "cerex-initial-head"
export const CREATED_AT_CONFIG = "cerex-created-at"
export const LEGACY_INTEGRATION_TARGET_CONFIG = "pinano-integration-target"
export const LEGACY_INITIAL_HEAD_CONFIG = "pinano-initial-head"
export const LEGACY_CREATED_AT_CONFIG = "pinano-created-at"
...
const INTEGRATION_TARGET_COMPARISON_SOURCE = "integration-target"
...
function repositoryRootFromManagedWorktreePath(path) {
	const normalized = resolve(path)
	for (const directory of [PRODUCT_STATE_DIRECTORY, LEGACY_PRODUCT_STATE_DIRECTORY]) {
		const marker = `${sep}${directory}${sep}wt${sep}`
...
async function gitCommandBaseForWorktree(record, options = {}) {
	...
	const repositoryRoot = stringOrUndefined(record?.repositoryRoot) ?? repositoryRootFromManagedWorktreePath(path)
...
async function integrationTargetConfig(path, parsedStatus, options = {}) {
	...
	const target = await runGitOutput(["-C", path, "config", "--get", `branch.${branch}.${INTEGRATION_TARGET_CONFIG}`], options)
		?? await runGitOutput(["-C", path, "config", "--get", `branch.${branch}.${LEGACY_INTEGRATION_TARGET_CONFIG}`], options)
	const base = localBranchBase(target)
	if (!base) return undefined
	const initialHead = stringOrUndefined(
		await runGitOutput(["-C", path, "config", "--get", `branch.${branch}.${INITIAL_HEAD_CONFIG}`], options)
		?? await runGitOutput(["-C", path, "config", "--get", `branch.${branch}.${LEGACY_INITIAL_HEAD_CONFIG}`], options),
	)
...
async function gitWorktreeStatusSnapshot(path, options = {}) {
	...
	const comparison = await firstWorkingComparison(path, [integrationTarget.base], INTEGRATION_TARGET_COMPARISON_SOURCE, options)
...
async function closeTrackedGitWorktree(record, terminalState, options = {}, preloadedStatus = undefined) {
	...
	const cleanupError = cleanupFailureMessage(cleanup)
	return {
		status,
		terminalState,
		...(head !== undefined ? { head } : {}),
...
export async function cleanupGitWorktreeRecords(records, options = {}) {
	const removed = []
	for (const record of records.filter((candidate) => candidate.operation === GIT_WORKTREE_ADD_OPERATION)) {
...
export async function cleanupSessionGitWorktrees(session, options = {}) {
	if (typeof session?.appendCustomEntry !== "function") return []
	const records = sessionGitWorktreeRecords(session)
	const removed = options.workspace
		? await options.workspace.worktrees.cleanup(records)
		: await cleanupGitWorktreeRecords(records, options)
	for (const event of removed) await session.appendCustomEntry(GIT_WORKTREE_CUSTOM_TYPE, event)
	return removed
}

export async function closeGitWorktreeRecords(records, payload, options = {}) {
	...
	const record = records.find((candidate) => candidate.path === request.path)
	if (!record) throw Object.assign(new Error("worktree is not tracked as an open Cerex worktree in this session"), { status: 404 })
	if (record.operation !== GIT_WORKTREE_ADD_OPERATION) {
		...
		return { event: record, status: terminalWorktreeStatus(record), alreadyClosed: true, cleanup: { ok: true, alreadyClosed: true }, cleanupOk: true }
	}
...
export async function closeSessionGitWorktree(session, payload, options = {}) {
	...
	const records = sessionGitWorktreeRecords(session)
	const close = options.workspace
		? await options.workspace.worktrees.close(records, payload, options.workerContext)
		: await closeGitWorktreeRecords(records, payload, options)
	if (!close) return undefined
	if (close.alreadyClosed) return { ...close, entryId: undefined }
...
export async function sessionGitWorktreeStatuses(session, options = {}) {
	...
	return options.workspace
		? options.workspace.worktrees.statuses(records, options)
		: gitWorktreeStatusesFromRecords(records, options)
}

...
const metadataFileName = ".runtime-source-reference.json"
const generationRetentionCount = 3
const defaultPackageEntries = ["bin", "cli", "packages", "README.md", "jsconfig.json"]
const optionalPrivatePackageEntries = ["packages/web"]
...
async function readPackageEntries(packageRoot) {
	try {
		...
		for (const entry of optionalPrivatePackageEntries) {
			if (await pathExists(join(packageRoot, entry))) optionalEntries.push(entry)
		}
		return [...new Set([
			"package.json",
			...files
...
/**
 * Materialize a read-only, code-only copy of the Cerex package that owns the
...
export async function ensureRuntimeSourceReference(identity) {
	if (!identity?.packageRoot) throw new Error("Cannot create Cerex runtime source reference without packageRoot")

...
const PINANO_INITIAL_HEAD_CONFIG = "pinano-initial-head"
...
function normalizeGitError(err) {
	...
	const message = gitLfs
		? "Git LFS is required by this repository but is unavailable to the Pinano service. Working-tree status and actions are disabled; commit history remains available."
		: "A required Git filter is unavailable to the Pinano service. Working-tree status and actions are disabled; commit history remains available."
...
async function repoBranch(root) {
	const branch = await git(["rev-parse", "--abbrev-ref", "HEAD"], root).then((out) => out.trim()).catch(() => "")
	const shortHead = await git(["rev-parse", "--short", "HEAD"], root).then((out) => out.trim()).catch(() => "")
	return {
...
async function repoWorktreeBase(root, branch) {
	...
	const initialHead = await git(["config", "--get", `branch.${branch}.${PINANO_INITIAL_HEAD_CONFIG}`], root)
		.then((out) => out.trim())
		.catch(() => "")

// Live runtime for one opened Pinano session.
...
import { isProjectContextMessage, loadProjectContextForCwd } from "../project-context.js"
import { compact, summarizeMessages } from "../compaction.js"
import { modelRef, resolveModel, sessionModelEligibilityError } from "../models.js"
...
import { completeEnvironmentPatch, getEnvironment, loadEnvironmentRegistry } from "../environments.js"
...
import { activeContextFiles, buildModelMessagesForSession, contextFilesDisabledForAgent, conversationEntriesForModel, projectAutomatedMaintenanceMessages } from "../session-context.js"
...
import { buildSkillsContextForPrompt, mergeSkillsContext, prependSkillsContext } from "../skills.js"
...
import { GIT_WORKTREE_ADD_OPERATION, GIT_WORKTREE_CLOSE_OPERATION, GIT_WORKTREE_CUSTOM_TYPE, GIT_WORKTREE_TERMINAL_APPLIED, PINANO_WORKTREE_EVENT_ROUTE, closeSessionGitWorktree, normalizeGitWorktreeEventPayload, sessionGitWorktreeStatuses, sessionOpenGitWorktreeRecords } from "../git-worktree-events.js"
import { internalHttpJsonResponse } from "../worker-internal-http.js"
import { PINANO_BRIDGE_SESSION_ROUTE } from "../pinano-bridge-session.js"
...
import {
	PINANO_PRE_TURN_MAINTENANCE_PLACEMENT,
	PINANO_PRE_TURN_SESSION_PROPERTIES_MAX_TOOL_CALLS,
...
export class SessionRuntime {
	 ...
	 * @param {import("../workspace-root-policy.js").WorkspaceRootPolicy} [options.workspacePolicy]
	...
	constructor(options) {
		...
		this.agent.pinanoSkillsContext = () => this.currentSkillsContext
	...
	async repairStaleToolCwd(options = {}) {
		...
		for (const check of checks) {
			if (!await pathIsExistingDirectory(check.path)) missingPaths.push(check)
		}
	...
	async flushPendingCwdFallbackNotices() {
		...
		for (const { notice, context } of pending) {
			const fallback = notice?.pinanoCwdFallback
	...
	async storedSessionEntryAllowed(entry) {
		return !this.workspacePolicy || (
			await this.workspacePolicy.allowsStoredCwd(entry?.cwd)
			&& await this.workspacePolicy.allowsStoredCwd(projectCwdForSessionEntry(entry))
		)
	}

	async pinanoSessionInfo(id) {
		const entry = this.db.getSession(id)
		if (entry && !await this.storedSessionEntryAllowed(entry)) {
			throw Object.assign(new Error(`session cwd is outside configured service.workspaceRoot (${this.workspacePolicy.root}): ${entry.cwd}`), { status: 403 })
		}
	...
	async bridgeProjectInfo() {
		return projectInfoForCwd(this.projectCwd())
	}

	async bridgeProjectSet(name, options = {}) {
		const result = await setProjectNameForCwd(this.projectCwd(), name)
	...
	replaceSkillsContextForText(text) {
		this.currentSkillsContext = buildSkillsContextForPrompt(this.skillsCwd(), text)
		return this.currentSkillsContext
	}

	mergeSkillsContextForText(text) {
		this.currentSkillsContext = mergeSkillsContext(
			this.currentSkillsContext,
			buildSkillsContextForPrompt(this.skillsCwd(), text),
		)
		return this.currentSkillsContext
	}
	...
	async handleInternalBridgeSessionRequest(request) {
		...
		if (payload.operation === "session.get") {
			const session = this.sessionBridge?.get
				...
				: await this.pinanoSessionInfo(sessionId)
		...
		if (payload.operation === "session.lifecycle") {
			...
			await this.invalidateSnapshot(this.sessionId)
			const session = await this.pinanoSessionInfo(this.sessionId)
			if (!session) throw Object.assign(new Error(`Session not found: ${sessionId}`), { status: 404 })
	...
	async handleInternalHttpRequest(request, workerContext = undefined) {
		...
		if (url.pathname === PINANO_BRIDGE_SESSION_ROUTE) return this.handleInternalBridgeSessionRequest(request)
	...
	async handlePinanoApiRequest(request) {
		...
		if (request.op === "session.get") {
			const info = await this.pinanoSessionInfo(id)
	...
	async handleAgentEvent(event) {
		...
		if (event.type === "message_start" && event.message?.role === "user") {
			...
			this.automatedMaintenanceKind = this.automatedMaintenanceTurnActive ? event.message.pinanoMaintenance : undefined
		}
		if (event.message && this.automatedMaintenanceTurnActive) {
			event.message.pinanoAutomated = true
			if (this.automatedMaintenanceKind) event.message.pinanoMaintenance = this.automatedMaintenanceKind
		}
	...
	async automatedMaintenancePreTurn(ctx) {
		...
		const props = this.effectiveSessionProperties()
		const project = await projectInfoForCwd(projectCwdForSnapshot(this.session, props, this.cwd)).catch(() => undefined)
	...
	projectMessagesForNextAction(ctx) {
		if (ctx.message?.role !== "assistant" || !ctx.message?.pinanoAutomated) return undefined
		const activeMaintenance = activeAutomatedMaintenanceUser(ctx.context?.messages ?? [])
		if (activeMaintenance?.pinanoMaintenancePlacement !== PINANO_PRE_TURN_MAINTENANCE_PLACEMENT) return undefined
		this.automatedMaintenanceTurnActive = false
		this.automatedMaintenanceKind = undefined
	...
	async automatedMaintenanceFollowUp(ctx) {
		if (this.isProjectMaintenanceSession()) return []
		if (this.agent.state.model?.provider === "mock") return []
		if (ctx.message?.pinanoAutomated || ctx.message?.pinanoMaintenance) return []
		...
		const hadPreTurnMaintenance = ctx.newMessages?.some((message) => message?.pinanoMaintenancePlacement === PINANO_PRE_TURN_MAINTENANCE_PLACEMENT)
		...
		if (openWorktreeRecords.length > 0 && (usedMeaningfulTool || props.state === SESSION_DISCUSSING_STATE)) {
			const worktrees = await sessionGitWorktreeStatuses(this.session).catch(() => [])
	...
	async snapshot(options = {}) {
		...
		const projectCwd = projectCwdForSnapshot(this.session, properties, this.cwd)
		const project = await projectInfoForCwd(projectCwd)
		const subSessions = await this.subSessions.list(this.sessionId, { includeClosed: true })
		const snapshot = {
	...
	startStreamingPrompt(userMessage, streamingBehavior) {
		...
		this.mergeSkillsContextForText(textFromContent(userMessage.content))
	...
	startPromptRun(message, userMessage) {
		...
		this.replaceSkillsContextForText(message)
		this.currentPrompt = {
			runId,
			text: message,
	...
	async startContinuationRun(options = {}) {
		...
		this.replaceSkillsContextForText(this.latestHumanPromptText())
		let run
		try {
	...
	pruneIdleResources(now = Date.now()) {
		...
		executor.pruneIdleWorkers(now).catch((err) => {
			process.stderr.write(`[pinano-runtime:${this.sessionId}] idle worker prune failed: ${err?.stack ?? err}\n`)
		})
	}

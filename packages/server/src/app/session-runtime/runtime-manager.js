// Manager for live Pinano session runtimes.
...
import { resolveModelWithProviderMetadata } from "../models.js"
import { assertNoSessionWorkspaceNameCollisions, branchSessionInDb, createSessionIdInDb, createSessionInDb, loadSessionPreviewInDb, openSessionInDb, restoreSessionInDb, sessionPreviewFromMessages } from "../session-store.js"
import { sessionActivityAt } from "../session-activity.js"
import { initialSessionEnvironment } from "../environments.js"
import { PREVIEW_FILE_SUFFIX, PREVIEW_LOG_DIRNAME, PREVIEW_ROOT_KIND_PROJECT, PREVIEW_ROOT_KIND_SOURCE, PREVIEW_ROOT_KIND_STATIC, STATIC_PREVIEW_NAME, matchPreviewHost, previewFileDefinitionFromPath, previewLogPath, previewPublicUrlFromSettings, previewRoutingSlugFromSettings, projectPreviewDirectory, projectPreviewLogPath, projectPreviewScopeId, readProjectPreviewDefinitions, readSessionPreviewDefinitions, sessionPreviewScopeId, sourcePreviewScopeId, staticPreviewDefinition, staticPreviewScopeId } from "../preview/manifest.js"
...
import { createWorkspaceRootPolicy } from "../workspace-root-policy.js"
...
import { GIT_WORKTREE_CUSTOM_TYPE, cleanupSessionGitWorktrees, gitWorktreeRecordsFromEntries, gitWorktreeStatusesFromRecords, resetInheritedSessionGitWorktrees, sessionGitWorktreeStatuses } from "../git-worktree-events.js"
import {
...
import {
	...
	projectPreviewsDirectoryExists,
...
export class RuntimeManager {
	 ...
	 * @param {import("../workspace-root-policy.js").WorkspaceRootPolicy} [opts.workspacePolicy]
	...
	constructor(opts, db, hub) {
		...
		this.workspacePolicy = opts.workspacePolicy
	...
	static async create(opts, db, hub) {
		const workspacePolicy = opts.workspacePolicy ?? await createWorkspaceRootPolicy(opts.workspaceRoot)
		const cwd = workspacePolicy ? await workspacePolicy.normalizeUserCwd(opts.cwd, "service startup cwd") : opts.cwd
		assertNoSessionWorkspaceNameCollisions(db)
		const manager = new RuntimeManager({ ...opts, cwd, workspacePolicy }, db, hub)
	...
	async normalizeUserCwd(cwd, label = "cwd") {
		return this.workspacePolicy ? await this.workspacePolicy.normalizeUserCwd(cwd, label) : cwd
	}

	async normalizeStoredCwd(cwd, label = "cwd") {
		return this.workspacePolicy ? await this.workspacePolicy.normalizeStoredCwd(cwd, label) : cwd
	}
	...
	async storedCwdAllowed(cwd) {
		if (!this.workspacePolicy || !cwd) return true
		return await this.workspacePolicy.allowsStoredCwd(cwd)
	}

	async assertStoredCwdAllowed(cwd, label = "session cwd") {
		if (await this.storedCwdAllowed(cwd)) return
		throw Object.assign(new Error(`${label} is outside configured service.workspaceRoot (${this.workspacePolicy.root}): ${cwd}`), { status: 403 })
	}
	...
	async workspaceAllowedSessionEntries(entries) {
		if (!this.workspacePolicy) return entries
	...
	async launchManagedSession(plan) {
		...
		if (plan.contextMessageCwd && this.opts.noContextFiles !== true) await ensureProjectContextMessage(opened.session, plan.contextMessageCwd)
	...
	async bridgeSessionInfo(id) {
		...
		if (runtime && !runtime.agent.isDead) {
			const info = await runtime.pinanoSessionInfo(id)
			if (!info) return undefined
			const properties = runtime.effectiveSessionProperties()
			return { ...info, project: await projectInfoForCwd(projectCwdForSnapshot(runtime.session, properties, runtime.cwd)) }
		}
		...
		try {
			...
			project = await projectInfoForCwd(projectCwdForSnapshot(opened.session, properties, entry.cwd))
		} catch (err) {
	...
	async bridgeSessionSet(id, patch, source = undefined) {
		...
		const properties = runtime.effectiveSessionProperties()
		return { ...info, project: await projectInfoForCwd(projectCwdForSnapshot(runtime.session, properties, runtime.cwd)), write }
	}
	...
	async bridgeSessionCollection(filters = {}) {
		...
		return Promise.all(rows.map(async (entry) => ({
			...entry,
			project: await projectInfoForCwd(projectCwdForSessionEntry(entry)).catch(() => undefined),
		})))
	}
	...
	async bridgeProjectInfo(id) {
		...
		try {
			...
			return projectInfoForCwd(projectCwdForSnapshot(opened.session, properties, entry.cwd))
		} catch (err) {
			this.rethrowSessionNotFound(id, err)
		}
	}
	...
	async registerProjectRoot(projectDir, options = {}) {
		...
		const root = resolve(await realpath(normalizedRoot))
		const info = await stat(root)
		if (!info.isDirectory()) throw Object.assign(new Error(`project directory must be a directory: ${normalizedRoot}`), { status: 400 })
		await this.assertStoredCwdAllowed(root, "project directory")
		const setup = await projectMaintenanceSetupStatus(root)
		const project = setup.project
		if (options.force !== true && !await shouldAutoRegisterProjectRoot(root, project)) return undefined
		const projectScopeId = projectPreviewScopeId(root)
		const staticScopeId = staticPreviewScopeId(root)
		const projectRoot = projectMaintenanceEligible(setup)
			? this.db.upsertPreviewRoot({
			...
			: undefined
		const staticRoot = this.db.upsertPreviewRoot({
	...
	async worktreePreviewSession(projectDir, sessionId) {
		...
		let sessionCwd
		try {
			root = resolve(await realpath(projectDir))
			sessionCwd = resolve(await realpath(await this.normalizeUserCwd(entry.cwd, "preview session cwd")))
		} catch (err) {
	...
	async resolveStaticPreviewRequest(request = {}) {
		...
		let filePath
		try {
			filePath = resolve(await realpath(requested))
		} catch (err) {
		...
		const record = longestContainingRoot(this.db.listPreviewRoots(PREVIEW_ROOT_KIND_STATIC), filePath)
		if (!record) throw Object.assign(new Error("Static HTML preview requires a registered project root"), { status: 404 })
		await this.assertStoredCwdAllowed(record.rootPath, "static preview project root")
	...
	async resolvePreviewSourcePath(value) {
		...
		if (!sourcePath.endsWith(PREVIEW_FILE_SUFFIX)) throw Object.assign(new Error(`preview source file must end with ${PREVIEW_FILE_SUFFIX}`), { status: 400 })
		const info = await stat(sourcePath)
		if (!info.isFile()) throw Object.assign(new Error("Preview source is not a file"), { status: 400 })
		await this.assertStoredCwdAllowed(dirname(sourcePath), "preview source directory")
		return sourcePath
	}

	async previewDefinitionForSourcePath(sourcePath) {
		try {
	...
	async resolvePreviewSource(request = {}) {
		const sourcePath = await this.resolvePreviewSourcePath(request.path)
		const definition = await this.previewDefinitionForSourcePath(sourcePath)
		const scopeId = sourcePreviewScopeId(sourcePath)
		const target = request.projectDir
			? await this.sourcePreviewTargetForProject(request.projectDir, sourcePath, definition, scopeId, { sessionId: request.sessionId })
			: undefined
		const fallbackTarget = target ?? await this.sourcePreviewTargetForSession(request.sessionId, sourcePath, definition, scopeId)
	...
	async sourcePreviewTargetFromRootRecord(rootRecord, match) {
		const sourcePath = rootRecord.rootPath
		await this.assertStoredCwdAllowed(dirname(sourcePath), "preview source directory")
		const definition = await this.previewDefinitionForSourcePath(sourcePath)
	...
	async ensureProjectMaintenanceSession(projectDir, options = {}) {
		...
		if (setup.needed && options.ensureSetupFiles !== false) await ensureProjectPreviewsDirectory(root)
		if (existing) {
			...
			if (entry) {
				...
				const project = setup.project ?? await projectInfoForCwd(root).catch(() => undefined)
					...
					if (projectSetupMaintenanceActive(runtime.agent.state.messages)) {
						...
						try {
							...
							const submission = await runtime.beginUserMessagePrompt({
								...
								pinanoMaintenance: PROJECT_SETUP_MAINTENANCE_KIND,
							}, undefined)
	...
	async resolvePreviewTarget(match) {
		...
		if (match?.name === STATIC_PREVIEW_NAME) {
			if (rootRecord?.scopeKind === PREVIEW_ROOT_KIND_STATIC) {
				await this.assertStoredCwdAllowed(rootRecord.rootPath, "static preview directory")
				return {
					scopeKind: "static",
					scopeId: value,
	...
	async previewTargetForUrl(value) {
		...
		if (url.protocol !== publicBase.protocol || urlPort(url) !== urlPort(publicBase)) throw Object.assign(new Error("URL is not a Pinano preview URL"), { status: 404 })
		const match = matchPreviewHost(url.host, publicUrl)
		if (!match) throw Object.assign(new Error("URL is not a Pinano preview URL"), { status: 404 })
	...
	async previewLogForUrl(value) {
		...
		return {
			preview: previewItemFromTarget(target, publicUrl, match.routingSlug, url.href),
			text: await readTextFileTail(target.logPath),
		}
	}
	...
	async bridgePreviewList(id) {
		...
		if (sessionCwdContext?.setup?.linkedGitWorktree) {
			...
			const projectDefinitions = Object.values((await readProjectPreviewDefinitions(projectDir)).previews)
			await this.registerProjectRoot(projectDir, { force: projectDefinitions.length > 0 })
			const projectOwner = await this.projectMaintenanceRecordForProject(projectDir)
			if (projectOwner.eligible) {
				const projectScope = { kind: "project", scopeId: projectPreviewScopeId(projectDir), sessionId: projectOwner.record?.sessionId, logPath: undefined }
				projectPreviews = projectDefinitions.map((definition) => previewItemFromDefinition({
					...projectScope,
					logPath: projectPreviewLogPath(projectDir, definition.name),
				}, definition, publicUrl, routingSlug))
			}
		}
		return {
	...
	async projectPreviewList(projectDir, options = {}) {
		...
		const publicUrl = this.opts.getPreviewPublicUrl?.() ?? previewPublicUrlFromSettings(this.opts.getSettings?.())
		const routingSlug = previewRoutingSlugFromSettings(this.opts.getSettings?.())
		const previewsDirectory = projectPreviewDirectory(root)
		const previewsDirectoryExists = await projectPreviewsDirectoryExists(root)
		const context = await this.projectMaintenanceContext(root)
		...
		if (previewsDirectoryExists) {
			try {
				projectDefinitions = Object.values((await readProjectPreviewDefinitions(root)).previews)
			} catch (err) {
				previewsError = /** @type {any} */ (err)?.message ?? String(err)
			}
		}
		...
		return {
			...
			previews: runner
				? projectDefinitions.map((definition) => previewItemFromDefinition({
					kind: "project",
					scopeId: projectScopeId,
					sessionId: runner.sessionId,
	...
	async createSession(cwd = this.cwd) {
		...
		const projectDir = await initialProjectDirForCwd(initial.cwd)
	...
	async projectSessionListEntries(entries, now = Date.now()) {
		...
		const projectsByCwd = new Map(await Promise.all(projectCwds.map(async (cwd) => [cwd, await projectInfoForCwd(cwd)])))
	...
	async overviewCwd(cwd = undefined) {
		...
		try {
			...
			if (this.workspacePolicy && /** @type {any} */ (err)?.status) return this.workspacePolicy.root
			throw err
		}
	}
	...
	async overviewProject(cwd = undefined) {
		...
		const project = await projectInfoForCwd(projectCwd)
	...
	async loadWorktreesUncached(id) {
		...
		try {
			...
			return gitWorktreeStatusesFromRecords(records)
		} catch (/** @type {any} */ err) {

...
import { getEnvironment, loadEnvironmentRegistry, resolveConfiguredEnvironmentId } from "../environments.js"
...
import { internalHttpRequestBodyText } from "../worker-internal-http.js"
...
import { worktreeStatusTtlMsForState } from "../worktree-status-policy.js"
...
export function isSessionNotFoundError(err) {
	return err?.code === "ENOENT"
		|| err?.code === "PINANO_SESSION_NOT_FOUND"
...
export function subSessionNoticeMessage({ name, parentSessionId, childSessionId }) {
	return {
		...
		content: [{ type: "text", text: [
			`This is Pinano sub-session "${name}" (${childSessionId}), spawned from parent session ${parentSessionId}.`,
...
export async function initialProjectDirForCwd(cwd) {
	try {
		const root = resolve(await realpath(cwd))
		const project = await projectInfoForCwd(root)
		return await shouldAutoRegisterProjectRoot(root, project) ? root : undefined
	} catch {
		return undefined
	}
}
...
/** @param {string} path */
export async function pathIsExistingDirectory(path) {
	try {
...
export function cwdFallbackNoticeText(report) {
	const missing = report.missingPaths
		...
		.join("\n")
	return [
		"Pinano runtime notice: one or more previous local tool execution paths no longer exist.",
		missing,
		"",
		`Tool execution cwd and session sandbox base were reset to the session workspace: ${report.sessionDir}`,
		"Use `pinano session set cwd <absolute-path>` to choose a project cwd when ready.",
	].join("\n")
}
...
export function cwdFallbackNoticeMatches(message, report) {
	const fallback = message?.pinanoCwdFallback
	return message?.pinanoCwdFallbackNotice === true

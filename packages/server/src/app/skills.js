...
import { optionalPinanoHomePath } from "./paths.js"
...
function ancestorsRootFirst(cwd) {
	const dirs = []
...
export function defaultSkillRoots(cwd) {
	...
	const pinanoHome = optionalPinanoHomePath()
	if (pinanoHome) roots.push({ path: join(pinanoHome, SKILLS_DIR_NAME), scope: "user" })
...
/** Parse the small YAML subset Pinano needs from SKILL.md frontmatter. */
...
function discoverSkillsUnderRoot(root, outcome) {
...
/** @param {string} cwd @param {{ roots?: SkillRoot[] }} [options] @returns {SkillLoadOutcome} */
export function loadSkillsForCwd(cwd, options = {}) {
	...
	for (const root of roots) discoverSkillsUnderRoot(root, outcome)
...
export function skillInjectionMessage(skill) {
	const contents = readFileSync(skill.path, "utf-8")
	return {
		role: "user",
		content: [{
			type: "text",
...
export function buildSkillsContextForPrompt(cwd, promptText, options = {}) {
	...
	try {
		...
		loadOutcome = {
			skills: [],
			errors: [{ path: resolvedCwd, message: err?.message ?? String(err) }],
			roots: options.roots ?? [],
		}
	}
...
export function mergeSkillsContext(current, next) {
	...
	for (const message of next.skillMessages) {
		const path = message?.pinanoSkillPath ?? ""

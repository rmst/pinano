...
import { startWorkerInternalHttpBridge } from "../internal-http.js"
...
function registerPidFile() {
	...
	try {
		...
		console.error(`failed to register Cerex worker pid file: ${err?.message ?? err}`)
		process.exit(1)
	}
}

function installPreviewAccessToken(value) {
	delete process.env[PREVIEW_ACCESS_TOKEN_ENV]
	delete process.env[LEGACY_PREVIEW_ACCESS_TOKEN_ENV]
	if (typeof value !== "string" || !value) return
	process.env[PREVIEW_ACCESS_TOKEN_ENV] = value
	process.env[LEGACY_PREVIEW_ACCESS_TOKEN_ENV] = value
}

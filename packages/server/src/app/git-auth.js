...
import { getCredential, updateCredential } from "./auth.js"
...
export async function withGitAskPassCredential(credential, fn) {
	const dir = await mkdtemp(join(tmpdir(), "pinano-git-auth-"))
	...
	try {
		...
		await writeFile(askpassFile, [
			"#!/bin/sh",
			"case \"$1\" in",
			"*Username*) printf '%s\\n' \"${PINANO_GIT_USERNAME:-git}\" ;;",
			"*Password*) token=$(cat \"$PINANO_GIT_TOKEN_FILE\") || exit 1; printf '%s\\n' \"$token\" ;;",
			"*) printf '\\n' ;;",
			"esac",
			"",
		].join("\n"), { mode: 0o700 })
		...
		return await fn({
			...
			PINANO_GIT_USERNAME: credential.username,
			PINANO_GIT_TOKEN_FILE: tokenFile,
		})
	} finally {

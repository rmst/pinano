...
function projectMetadataMaintenanceInstruction(project) {
	...
	return `\n\nProject metadata for this session's project directory is missing. Choose a concise stable project name from the existing session context, use lowercase unless the user requested otherwise, and set it with \`pinano project set <name>\`.`
}
...
function projectDirMaintenanceInstruction(props) {
	...
	return "If `project-dir` is unset and the main project root is clear from context, set it with `pinano session set project-dir <absolute-path>`; prefer the stable repository root, not a short-lived worktree path. If unclear, leave it unset."
}
...
If \`description\` is missing, set it with \`pinano session set description <text>\`. ${projectDirInstruction} Otherwise update session metadata only when the current value is stale or misleading. Never include properties whose values should remain unchanged.${projectInstruction}
...
export function createWorktreeLifecyclePromptMessage(props = undefined, worktreeInfo = {}) {
	...
	return {
		role: "user",
		pinanoAutomated: true,
		pinanoMaintenance: SESSION_WORKTREE_LIFECYCLE_MAINTENANCE,
		content: [{
			type: "text",
...
Record the choice with exactly one lifecycle command: \`pinano session lifecycle <needs-input|ready-for-review>\`.

If the session description is missing, stale, or misleading, you may also update it with \`pinano session set description <text>\`. Otherwise do not update metadata.`,
		}],
		timestamp: Date.now(),
	}
}

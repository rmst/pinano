...
Pinano keeps the default TUI path dependency-free and build-free, so the TypeScript upstream source is converted to plain ESM JavaScript and committed here rather than imported from `node_modules`.

Conversion used:

```sh
git clone https://github.com/markedjs/marked.git /tmp/pinano-marked
cd /tmp/pinano-marked

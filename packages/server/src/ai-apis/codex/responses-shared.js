...
import { isResponsesCompactionItem, isResponsesNativeItemBlock, responsesNativeItemBlock } from "../../../../protocol/src/responses-compaction.js"
...
function responseItemPhase(item) {
	return item?.phase === "commentary" || item?.phase === "final_answer" ? item.phase : undefined
}

function responseTextBlockMetadata(item, fallbackPhase) {
	const phase = responseItemPhase(item) ?? fallbackPhase
	return {
		...(phase ? { phase } : {}),
		...(typeof item?.id === "string" && item.id ? { textSignature: encodeTextSignature(item.id, phase) } : {}),
	}
}
...
function assertImageData(block) {
	if (typeof block.data !== "string" || block.data.length === 0) {
		throw new Error("Image content block is missing base64 data")
	}
	if (typeof block.mimeType !== "string" || !block.mimeType.startsWith("image/")) {
		throw new Error("Image content block is missing an image MIME type")
	}
}

function responseInputImage(block, model) {
	assertImageData(block)
	return {
		type: "input_image",
 ...
 */
export async function processResponsesStream(events, output, stream, model, pricingContext = {}) {
	...
	for await (const event of events) {
		...
		if (event.type === "response.output_item.added") {
			...
			if (isResponsesCompactionItem(item)) {
				...
				currentItem = item
				currentBlock = { type: "text", text: "", ...responseTextBlockMetadata(item) }
				blocks.push(currentBlock)
		...
		if (event.type === "response.output_item.done") {
			...
			if (isResponsesCompactionItem(item)) {
				...
				const metadata = responseTextBlockMetadata(item, currentBlock.phase)
				if (metadata.phase) currentBlock.phase = metadata.phase
				else delete currentBlock.phase
				if (metadata.textSignature) currentBlock.textSignature = metadata.textSignature
				else delete currentBlock.textSignature
				stream.push({
					type: "text_end",
					contentIndex: blockIndex(),
		...
		if (event.type === "response.completed") {
			...
			if (response?.usage) output.usage = normalizeResponsesUsage(response.usage, model, pricingContext)

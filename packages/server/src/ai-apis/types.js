 ...
 * @property {"commentary" | "final_answer"} [phase]
 */
...
/**
 * Dynamic pricing in USD per million tokens. Used when price depends on
 * service tier and/or the provider's raw input-token count.
 * @typedef {object} ModelPricing
 * @property {number} [longContextThresholdInputTokens]
 * @property {Record<string, { short?: ModelCost, long?: ModelCost, longContextThresholdInputTokens?: number }>} serviceTiers
 */
 ...
 * @property {"auto" | "sse" | "websocket"} [codexTransport] Preferred transport for ChatGPT subscription Responses calls.
 ...
 * @property {number|false|null} [firstStreamEventTimeoutMs] Override timeout while waiting for the first parsed SSE event after response headers.
 * @property {number|false|null} [streamEventInactivityTimeoutMs] Override timeout while waiting for parsed SSE events after the first event.
 * @property {number|false|null} [webSocketConnectTimeoutMs] Override timeout while opening a Codex WebSocket.
 * @property {"auto" | "sse" | "websocket"} [codexTransport] Prefer WebSocket with fallback, force SSE, or force WebSocket.
 * @property {string} [serviceTier]
 * @property {boolean} [compactionTrigger] Append an explicit Responses remote-v2 compaction trigger.
 ...
 * @property {"none" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max"} [reasoningEffort]
 * @property {"low" | "medium" | "high"} [textVerbosity]
 * @property {Function} [webSocketConstructor] Optional WebSocket constructor supplied by an embedding runtime.
 */

...
import { GIT_WORKTREE_CLOSE_OPERATION, GIT_WORKTREE_TERMINAL_APPLIED, GIT_WORKTREE_TERMINAL_DISCARDED, WORKTREE_EVENT_ROUTE } from "../app/source-control/worktree-events.js"
import { CliError, gitOutput, postInternalEvent } from "./worktree-command-utils.js"

const VERSION = "cerex-worktree-close 0.1"
...
Usage:
  cerex-worktree-close (--applied | --discarded) [--path <path>]

Closes a Cerex-tracked Git worktree, records its terminal lifecycle state,
...
function responseError(response) {
	...
	return [bodyError || response?.text || "Cerex could not close the worktree", ...details].filter(Boolean).join("\n")
}
...
export async function runManagedWorktreeClose(config, options = {}) {
	...
	if (!env[INTERNAL_API_BASE_URL_ENV] || !env[INTERNAL_API_TOKEN_ENV]) {
		throw new CliError("cerex-worktree-close must be run from a Cerex tool session")
	}
	...
	if (response.body?.recorded === false && response.body?.alreadyClosed !== true) throw new CliError("Cerex did not record the worktree close event", 1)
...
export async function main(argv = process.argv.slice(2), options = {}) {
	try {
		...
		await runManagedWorktreeClose(config, options)
		return 0
	} catch (err) {

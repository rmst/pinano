...
import { PROCESS_SESSION_IDLE_TTL_MS, PROCESS_SESSION_MAX_SESSIONS, PROCESS_SESSION_MAX_STORED_OUTPUT_BYTES } from "./process-session-limits.js"
...
const MAX_MODEL_VISIBLE_OUTPUT_TOKENS = DEFAULT_MAX_OUTPUT_TOKENS
const APPROX_BYTES_PER_OUTPUT_TOKEN = 4
export const SESSION_IDLE_TTL_MS = PROCESS_SESSION_IDLE_TTL_MS
export const MAX_STORED_OUTPUT_BYTES = PROCESS_SESSION_MAX_STORED_OUTPUT_BYTES
export const MAX_SESSIONS = PROCESS_SESSION_MAX_SESSIONS
...
export function maxOutputBytesForTokens(value) {
	...
	const effectiveTokens = Math.min(wholeTokens, MAX_MODEL_VISIBLE_OUTPUT_TOKENS)
	return Math.min(effectiveTokens * APPROX_BYTES_PER_OUTPUT_TOKEN, MAX_STORED_OUTPUT_BYTES)
}
...
export class ProcessSessionManager {
	...
	disposeAll(signal = "kill", options = {}) {
		signalName(signal)
		const sessions = [...this.sessions.values()]
		this.sessions.clear()
		for (const session of sessions) {
			const forceProcessGroup = session.running
			session.dispose(signal)
			const forceAfterMs = options.forceAfterMs
			if (signal !== "kill" && Number.isFinite(forceAfterMs) && forceAfterMs >= 0) {
				const timer = setTimeout(() => {
					if (forceProcessGroup) session.sendSignal("kill")
				}, forceAfterMs)
				timer.unref?.()
			}
		}

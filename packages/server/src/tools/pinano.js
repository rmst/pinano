...
import { PINANO_INTERNAL_API_BASE_URL_ENV, PINANO_INTERNAL_API_TOKEN_ENV, PINANO_SESSION_ENV } from "../../../protocol/src/internal-api-env.js"
import { PINANO_BRIDGE_SESSION_ROUTE } from "../app/pinano-bridge-session.js"
import { PREVIEW_AUTHORIZATION_HEADER } from "../app/preview/manifest.js"
import { CliError, postInternalEvent } from "./pinano-worktree-command-utils.js"

const VERSION = "pinano 0.1"
...
Usage:
  pinano session [--id <session-id>] get
  pinano session [--id <session-id>] set <property> <value> [<property> <value> ...]
  pinano session lifecycle <needs-input|ready-for-review>
  pinano session [--id <session-id>] cat [<entry>|<from>:<to>] [--full]
  pinano sessions [--state <state>] [--since <time>] [--limit <n>]
  pinano sessions cat (--all | --state <state> | --since <time> | --limit <n>) [--full]
  pinano project
  pinano project get
  pinano project set <name>
  pinano preview

Without --id, pinano session reads the target session id from $${PINANO_SESSION_ENV}.
Lifecycle updates are current-session-only and are accepted only during automated worktree lifecycle maintenance.
pinano sessions lists visible sessions. pinano sessions cat concatenates visible transcripts for a filtered collection.

Project commands update project metadata for the current session's project directory.

Always use pinano preview when trying to expose, inspect, or verify a web server, web app, docs server, or static HTML preview for the user. It prints configured preview names, scopes, concrete URLs, source files, and log paths; use the printed URLs instead of constructing preview hostnames.

Preview modules:
  - Project previews live under .pinano/previews/<name>.preview.js.
  - Temporary session previews live under $PINANO_SESSION_DIR/previews/<name>.preview.js.
  ...
  - Shell exec strings run in the owning session cwd with PINANO_HOST, PINANO_PORT, PINANO_PUBLIC_URL, and PINANO_PREVIEW_LOG.
  - JS exec functions are called as exec({ host, port, publicUrl, logPath, signal, env }).
  - The preview must listen on host:port or $PINANO_HOST:$PINANO_PORT.
  - After writing a preview module, run pinano preview and give the printed URL to the user.
...
Options:
  ...
  --all            explicitly allow pinano sessions cat to read all visible sessions
...
Preview modules:
  ...
  - JS exec functions are called as exec({ host, port, publicUrl, logPath, signal, env }).
  - The preview must listen on host:port or $PINANO_HOST:$PINANO_PORT.
  - Pinano starts each preview lazily on first request and writes stdout/stderr to PINANO_PREVIEW_LOG.
  - After writing or changing a preview module, run pinano preview and give the printed URL to the user.

Output columns:
  name<TAB>scope<TAB>url<TAB>source<TAB>log

Preview access:
  - To inspect a printed preview URL yourself from a Pinano tool session, request it with Authorization: Bearer $${PINANO_INTERNAL_API_TOKEN_ENV}.
  - If the previewed app itself needs an Authorization header, send that app header as ${PREVIEW_AUTHORIZATION_HEADER}; Pinano forwards it after preview access is authenticated.
  - This token is only for Pinano preview access; do not print its value, write it into files, or include it in user-facing messages.
...
Examples:
  pinano session cat
  pinano sessions cat --since 60d | rg -i compaction
  pinano session --id <session-id> cat 42
  pinano session --id <session-id> cat 40:45
  pinano session --id <session-id> cat 42 --full
...
function sessionIdFromEnv(env) {
	const sessionId = String(env[PINANO_SESSION_ENV] ?? "").trim()
	if (!sessionId) throw new CliError(`${PINANO_SESSION_ENV} is not set`, 2)
	return sessionId
}
...
export function parseArgs(args) {
	...
	if (args[0] === "preview") {
		...
		if (args.length > 1) throw new CliError("pinano preview does not take arguments", 2)
...
function responseError(response) {
	...
	return bodyError || response?.text || "Pinano bridge request failed"
}
...
function formatSetResult(body = {}) {
	...
	if (unchanged.length > 0) {
		const prefix = changed.length === 0
			? "Warning: this pinano session set command was unnecessary because all supplied fields were unchanged"
...
async function bridgeRequest(env, body, options = {}) {
	if (!env[PINANO_INTERNAL_API_BASE_URL_ENV] || !env[PINANO_INTERNAL_API_TOKEN_ENV]) {
		throw new CliError("pinano bridge commands must be run from a Pinano tool session")
	}
	const response = await postInternalEvent(env[PINANO_INTERNAL_API_BASE_URL_ENV], env[PINANO_INTERNAL_API_TOKEN_ENV], PINANO_BRIDGE_SESSION_ROUTE, body, { timeoutMs: options.timeoutMs ?? REQUEST_TIMEOUT_MS })
...
export async function runPinanoCommand(config, options = {}) {
	...
	if (config.namespace === "project") {
		...
		const body = await bridgeRequest(env, {
			...
			sessionId,
			...(config.operation === "set" ? { name: config.name, toolCallId: env.PINANO_TOOL_CALL_ID || undefined } : {}),
		})
...
export async function main(argv = process.argv.slice(2), options = {}) {
	try {
		...
		await runPinanoCommand(config, options)
		return 0
	} catch (err) {

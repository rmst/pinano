...
import { createPromptImageContentAutoResize, promptImageWasResized } from "./image-prompt.js"
import { detectImageMime } from "./mime.js"
import { resolveToCwd } from "./path-utils.js"
import { DEFAULT_MAX_BYTES, DEFAULT_MAX_LINES, formatSize, truncateHead } from "./truncate.js"

const TEXT_DESC = `Read the contents of a file. Supports text and images (png, jpg, gif, webp); images are returned as attachments. High-detail images are resized to prompt limits when possible; use detail=original to preserve image resolution when supported. Text output is truncated to ${DEFAULT_MAX_LINES} lines or ${DEFAULT_MAX_BYTES / 1024}KB (whichever is hit first). Use offset/limit for large text files; continue with offset to read the rest.`
...
export function createReadTool(cwd) {
	return {
		...
		async execute(_id, { path, offset, limit, detail }, signal) {
			...
			if (mimeType) {
				const image = await createPromptImageContentAutoResize(buffer, { path, detail })
				const resizeText = promptImageWasResized(image)
					? `, resized from ${image.original.widthPx}×${image.original.heightPx}px ${image.original.mimeType}`
					: ""
				return {
					content: [
						{ type: "text", text: `Read image file [${image.mimeType}, ${image.widthPx}×${image.heightPx}px, detail=${image.detail}${resizeText}]` },
						image,
					],
					details: {

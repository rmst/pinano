...
export function startHttpAttempt(request, { attemptIndex = 0, method = "POST", url, headers, body }) {
	if (!request?.id) return null
	return withLog((s) => {
		const id = randomUUID()
		const bodyKind = typeof body === "string" && body === request.requestJson ? "model_request" : "inline"
		s.insertAttempt.run(
			id,
			request.id,
			attemptIndex,
			...
			bodyKind,
			bodyKind === "model_request" ? null : body,
		)

...
export function shouldCompact(agent, threshold) {
	...
	const estimated = estimateTokens(/** @type {any} */ (agent.state.messages))
	...
	return messages
		.map((m) => {
			...
			const text = (m.content ?? [])
				...
				.join(" ")
			if (role === "assistant" && m.content?.some?.((/** @type {any} */ c) => c.type === "toolCall")) {
				const calls = m.content.filter((/** @type {any} */ c) => c.type === "toolCall").map((/** @type {any} */ c) => c.name)
...
export async function compact(agent, keepLast = 6, signal) {
	...
	agent.state.messages = /** @type {any} */ ([compactionMsg, ...tail])

	...
	render(width) {
		...
		for (let i = startIndex; i < endIndex; i++) {
			...
			const activeMarker = node.entry.kind === "leaf"
				? node.entry.active ? theme.fg("accent", "◆ ") : theme.fg("muted", "◇ ")
				: node.entry.onActivePath ? theme.fg("accent", "• ") : "  "
...
export class HistorySelectorComponent extends Container {
	...
	render(width) {
		...
		let lines = this.renderChildrenIncremental(width).lines
		if (desiredHeight > 0) {
			if (lines.length < desiredHeight) {
				const insertAt = Math.max(0, lines.length - 1)
				lines = [
					...lines.slice(0, insertAt),
					...Array.from({ length: desiredHeight - lines.length }, () => ""),

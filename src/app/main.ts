...
import { buildProjectContextMessage, ensureProjectContextMessage } from "./project-context.ts"
import {
	createSession,
	getLatestForCwd,
	openSession,
...
import { SlashCommandRegistry } from "./slash-commands.ts"
import { registerSimpleCommands } from "./commands/simple.ts"
import { registerOverlayCommands } from "./commands/overlays.ts"
import { compactCommand } from "./commands/compact.ts"
import { registerBranchingCommands } from "./commands/branching.ts"
import { compact, shouldCompact } from "./compaction.ts"
import { runPrintMode } from "./print-mode.ts"
import { runRpcMode } from "./rpc-mode.ts"
import { installStderrCapture } from "./stderr-capture.ts"
...
function buildStreamFn() {
	let codexCreds: any
	return async (model: any, ctx: any, options: any) => {
...
function systemPromptFor(cwd: string): string {
	const now = new Date()
...
async function main() {
	...
	let preResolved: { session: any; id: string; cwd: string; path: string } | undefined
	if (args.session) {
		try {
	...
	const agent = new Agent({
		initialState: {
			model,
			thinkingLevel: settings.thinkingLevel as any,
	...
	if (args.print) {
		if (preResolved) {
			...
			agent.state.messages = preResolved.session.getMessages() as any
			agent.subscribe(async (event) => {
				if (event.type === "message_end") {
					try { await preResolved!.session.appendMessage(event.message as any) } catch {}
				}
			})
	...
	//   else          → start fresh
	let opened: { session: any; id: string }
	if (preResolved) {

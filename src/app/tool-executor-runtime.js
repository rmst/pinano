...
export class ToolExecutorRuntime {
	...
	resolveTarget() {
		...
		const sessionWd = resolveSessionWd(props, sandboxFallbackCwd(session, this.cwd), registry)
		...
		return { ...target, sessionWd, sessionDir, pinanoStateMount, workerScope: sessionId ? `session:${sessionId}` : "runtime" }
	}

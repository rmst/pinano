...
const deprecatedSessionWorktreesDir = "worktrees"
...
function pathStartsWithTopLevelName(rel, name) {
...
function branchPath(path, mappings) {
	...
	if (match) {
		const rel = relative(match.sourceDir, resolve(path))
		const insideDeprecatedWorktrees = pathStartsWithTopLevelName(rel, deprecatedSessionWorktreesDir)
		const newPath = insideDeprecatedWorktrees
			? match.targetDir
			: rel ? resolve(match.targetDir, rel) : match.targetDir
		return {
			oldPath: path,
			newPath,
			changed: newPath !== path,
			reason: insideDeprecatedWorktrees ? "deprecated-session-worktree" : "session-workspace",
		}
	}
...
async function copySessionFiles(sourceDir, targetDir) {
	...
	for (const entry of await readdir(sourceDir, { withFileTypes: true })) {
		if (entry.name === deprecatedSessionWorktreesDir) continue

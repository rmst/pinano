...
import { codexUsageBaseUrlFromSettings, codexUsageUrl, fetchCodexUsage } from "./codex-usage.js"
...
export function startCodexUsagePoller(options = {}) {
	...
	const run = () => {
		...
		running = pollCodexUsageOnce({ ...options, signal: controller.signal })
			.catch((error) => {
				if (!closed && process.env.PINANO_TEST !== "1") console.error(`codex usage poll failed: ${errorMessage(error)}`)

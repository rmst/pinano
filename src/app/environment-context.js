...
function effectiveMountsForContext(environment, initialCwd, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts = []) {
...
function containerFilesystemLines(environment, initialCwd, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts = []) {
	const sandbox = environment.sandbox
...
function nativeFilesystemLines(environment, initialCwd, platform, environmentHomePath, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts = []) {
	const mounts = effectiveMountsForContext(environment, initialCwd, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts)
...
function filesystemLines(environment, initialCwd, platform, environmentHomePath, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts = []) {
	const sandbox = environment?.sandbox
	...
	if (sandbox.type === "native") return nativeFilesystemLines(environment, initialCwd, platform, environmentHomePath, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts)
	if (sandbox.type === "container") return containerFilesystemLines(environment, initialCwd, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts)
	return [`Filesystem: tool sandbox is ${sandbox.type}.`]
}

function environmentBody(environment, initialCwd, platform, arch, environmentHomePath, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts = []) {
	return [
		`<environment ${environmentAttrs(environment)}>`,
		`Runs: ${runsDescription(environment, platform, arch)}.`,
		...filesystemLines(environment, initialCwd, platform, environmentHomePath, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts),
		`</environment>`,
	].filter(Boolean)
}

export function sessionWorkspaceToolPathForEnvironment(environment, initialCwd, runtimeSourceReferencePath, workspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts = []) {
	...
	const mounts = effectiveMountsForContext(environment, initialCwd, runtimeSourceReferencePath, workspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts)
...
function sessionWorkspaceLines(environment, initialCwd, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts = []) {
	const path = sessionWorkspaceToolPathForEnvironment(environment, initialCwd, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts)
	if (!path) return []
	return [
		"Session workspace:",
...
export function environmentContextFor(options = {}) {
	...
	const lines = [
		...
		...sessionWorkspaceLines(currentEnvironment, initialCwd, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts),
		...environments.flatMap((environment) => environmentBody(environment, initialCwd, platform, arch, environmentHomePath, runtimeSourceReferencePath, sessionWorkspacePath, pinanoStateDirPath, pinanoStateMount, sessionSandboxMounts)),
		"</environment_context>",
	].filter(Boolean)
	return lines.join("\n")
}

...
import { loadProjectContextFiles, type ContextFile } from "./context-files.ts"
...
export function formatContextFilesAsUserMessage(files: ContextFile[], cwd: string): string {
...
export function buildProjectContextMessage(cwd: string): any | null {
...
export function isProjectContextMessage(m: any): boolean {
	if (m?.role !== "user") return false
	const content = m.content
	if (Array.isArray(content)) {
		return content.some(
			(c: any) => c?.type === "text" && typeof c.text === "string" && c.text.includes(PROJECT_CONTEXT_HEADING),
		)
	}
...
export function hasProjectContextMessage(messages: ReadonlyArray<any>): boolean {
 ...
 */
export async function ensureProjectContextMessage(session: any, cwd: string): Promise<void> {

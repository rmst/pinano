...
export async function postInternalEvent(baseUrl, token, route, body, options = {}) {
	if (!baseUrl || !token) return { ok: false, status: 0, text: "Pinano internal API environment is not available" }
	let url
	try {
		url = new URL(route, baseUrl)
	} catch {
		return { ok: false, status: 0, text: "invalid Pinano internal API URL" }
	}

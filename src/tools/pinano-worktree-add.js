...
import { GIT_WORKTREE_ADD_OPERATION, PINANO_CREATED_AT_CONFIG, PINANO_INITIAL_HEAD_CONFIG, PINANO_INTEGRATION_TARGET_CONFIG, PINANO_WORKTREE_EVENT_ROUTE } from "../app/git-worktree-events.js"
import { CliError, commandResult, gitOutput, postInternalEvent, run } from "./pinano-worktree-command-utils.js"

const VERSION = "pinano-worktree-add 0.1"

function usage() {
	return `${VERSION}

Usage:
  pinano-worktree-add [options] <name> [<start-point>]

Creates a short-lived Pinano-managed Git worktree. The worktree path defaults to
<repo>/.pinano/wt/<name>. Without <start-point>, the integration target is used.
...
Options:
  ...
  --copy                          copy ignored files even when copy-on-write is unavailable
...
function isLegacyBlanketPinanoIgnore(text) {
	const entries = text.split(/\r?\n/).map((line) => line.trim()).filter(Boolean)
	return entries.length === 1 && entries[0] === "*"
}

async function removeLegacyBlanketPinanoIgnore(pinanoDir) {
	const legacyGitignore = join(pinanoDir, ".gitignore")
	let text
	try {
		text = await readFile(legacyGitignore, "utf-8")
	} catch {
		return
	}
	if (isLegacyBlanketPinanoIgnore(text)) await rm(legacyGitignore, { force: true })
}

async function ensurePinanoWorktreeStorageIgnored(repo) {
	const pinanoDir = join(repo, ".pinano")
	const worktreeDir = join(pinanoDir, "wt")
	await mkdir(worktreeDir, { recursive: true })
	await removeLegacyBlanketPinanoIgnore(pinanoDir)
...
function shouldSkipIgnoredEntry(relPath, repo, worktreePath) {
	...
	if (normalized === ".git"
		...
		|| normalized === ".pinano/wt"
		|| normalized.startsWith(".pinano/wt/")) return true
...
async function copyPinanoDirectory(repo, worktreePath, canCow) {
	const pinanoDir = join(repo, ".pinano")
	const entries = await readdir(pinanoDir).catch(() => [])
	let copied = 0
	for (const entry of entries) {
		if (entry === "wt") continue
		const source = join(pinanoDir, entry)
		if (!(await pathExists(source))) continue
		if (isInside(source, worktreePath) || isInside(worktreePath, source)) continue
		await copyOne(source, join(worktreePath, ".pinano", entry), canCow)
		copied += 1
	}
	return copied
}
...
async function cowAvailable(repo, worktreePath) {
	const probeName = `.pinano/copy-probe-${process.pid}-${Date.now()}`
	const source = join(repo, probeName)
	const destination = join(worktreePath, probeName)
	try {
		await mkdir(dirname(source), { recursive: true })
		await mkdir(dirname(destination), { recursive: true })
		await writeFile(source, "pinano copy probe\n")
...
async function copyIgnoredFiles(repo, worktreePath, mode) {
	if (mode === "none") return { copied: 0, skipped: false }
	const entries = (await ignoredEntries(repo)).filter((entry) => {
		const relPath = entry.replace(/\/+$/, "")
		return relPath === ".pinano" || !shouldSkipIgnoredEntry(entry, repo, worktreePath)
	})
	if (entries.length === 0) return { copied: 0, skipped: false }
	const canCow = await cowAvailable(repo, worktreePath)
	if (!canCow && mode === "auto") {
		process.stderr.write("pinano-worktree-add: ignored files were not copied; use --copy to copy them.\n")
	...
	for (const entry of entries) {
		const relPath = entry.replace(/\/+$/, "")
		if (relPath === ".pinano") {
			copied += await copyPinanoDirectory(repo, worktreePath, canCow)
			continue
		}
...
async function recordWorktree(env, event) {
	...
	if (!response.ok && env[PINANO_INTERNAL_API_BASE_URL_ENV] && env[PINANO_INTERNAL_API_TOKEN_ENV]) {
		process.stderr.write("pinano-worktree-add: warning: worktree was created but could not be recorded by Pinano.\n")
	}
}

export async function runPinanoWorktreeAdd(config, options = {}) {
	...
	const worktreePath = resolve(config.path ? resolve(cwd, config.path) : join(repo, ".pinano", "wt", config.name))
	if (!config.path && !isInside(worktreePath, join(repo, ".pinano", "wt"))) {
		throw new CliError(`worktree name escapes .pinano/wt: ${config.name}`, 2)
	}

	await ensurePinanoWorktreeStorageIgnored(repo)
	...
	await git(["config", `branch.${branch}.${PINANO_INTEGRATION_TARGET_CONFIG}`, integrationTarget], worktreePath)
	await git(["config", `branch.${branch}.${PINANO_INITIAL_HEAD_CONFIG}`, initialHead], worktreePath)
	await git(["config", `branch.${branch}.${PINANO_CREATED_AT_CONFIG}`, createdAt], worktreePath)
...
export async function main(argv = process.argv.slice(2), options = {}) {
	try {
		...
		await runPinanoWorktreeAdd(config, options)
		return 0
	} catch (err) {
